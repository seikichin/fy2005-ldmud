	...
	if (me->query("private_maze/destination") == base_name(environment(me))+".c"
		...
		if (me->query("private_maze/create")){
			 ...
			 FUBEN_D->enter_fuben(me, "ultra");
              ...
              // /f/ultra/smaug/maze/entry
			room2=find_object("/f/ultra/"+me->query("id")+"/maze/entry");			
			if (!room2)	
			...
			room2->set("maze/boss",		"/maze/ultra/npc/boss");	
			room2->set("maze/blocker",	"/maze/ultra/npc/blocker");	
			room2->set("maze/npc_pfm_chance", me->query("private_maze/difficulty"));
			
			tell_object(me,sprintf("\n任务区域%s创建成功，允许进入最高等级为%s，最多人数%s人。\n",
...
//		me->set_temp("maze_entry",environment(me));	
		...
		if (who==me){			
		if(room2) FUBEN_D->enter_fuben(me, "ultra");
        	}
		
		tell_object(who, WHT"进入"+me->name()+"的区域：" + me->query("private_maze/orig_name")+ "，你的任务是彻底摧毁这里。\n"NOR);
		if (who!=me){
		      if(room2)
			who->move(room2);
