...
void more(string cmd, string *text, int line)
{
  ...
  write(show);
  input_to("more", INPUT_PROMPT, "",text, line);
}
...
void more_file(string cmd, string file, int line, int total)
{
  ...
  if (not_show || i > 1)
  {
    ...
    "显示接下 " HIC "n" NOR WHT " 行)\n" NOR;
    s_write(show);
    input_to("more_file", INPUT_PROMPT, "", file, line + page, total);
  } else
  {
    show += WHT "阅读完毕。\n" NOR;
    s_write(show);
...
{
  return sizeof(old_explode(file, "\n"));
}
...
int main(object me, string arg)
{
	...
	seteuid(geteuid(me));
	if (!arg) { notify_fail("指令格式 : more 档名|物件名 \n"); return 0;}
	file = resolve_path(me->query("cwd"), arg);
	if( file_size(file) < 0 ) {
		ob = present(arg, me);
		if( !ob ) ob = present(arg, environment(me));
		if( !ob ) { notify_fail("没有这个档案。\n");return 0;}
		file = base_name(ob) + ".c";
	}
...
int help(object me)
{
write(funcall(#'read_file,"/help/cmds/arch/more"));
    return 1;
}
