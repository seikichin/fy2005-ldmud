...
int sort_user(object o1,object o2);
int help();
...
mixed main(object me, string arg, int remote) {
	...
	ob = filter(children(USER_OB),(: userp($1) :));
	ob = filter(ob, (: environment($1) :));
	ob = filter(ob, (: $2->visiblep($1) :), me);
	numPlayer = sizeof(ob);
	str = HIY"\n◎ 「"+ MUD_NAME+"」"HIW"目前";
	rule = "";
	if(arg) {
		option = old_explode(arg, " ");
		i = sizeof(option);
		while( i-- ) {
			switch(option[i]) {
				case "-h": 
				...
				case "-w": 
					rule += "巫师";
					if (wizardp(me))
						ob = filter(ob, (: wizardp($1) || wizhood($1) == "(immortal)" :));
					else
						ob = filter(ob, (: (wizardp($1) || wizhood($1) == "(immortal)") 
						&& !$1->query("wiz_invis") :));
					break;
				case "-p": 
					...
					ob = filter(ob, (: $1->query("family/family_name") == $2->query("family/family_name") :), me);
					break;
				case "-f": 
					rule += "女性";
					ob = filter(ob, (: $1->query("gender") =="女性" :));
					break;
				case "-m": 
					rule += "男性";
					ob = filter(ob, (: $1->query("gender") == "男性" :));
					break;
				case "-a": 
					opt_number = 1;
					break;
				case "-d": 
					rule += "断线";
					ob = filter(ob, (: $1->query_temp("netdead") :));
					break;
				default:
