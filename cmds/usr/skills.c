...
string skill_level(string name, int skill_level);

int main(object me, string arg)
{
	object ob, couple_ob;
	mapping skl, lrn, map;
	string *sname, *mapped,target,skill_name, msg;
	int i, skill_level;
...
//	==> implemented to reduce flooding when player only needs to check a single skill level -- by silencer
	...
	if (sscanf(arg,"-%s -%d",skill_name,estimate)==2 
		|| sscanf(arg,"-%s",skill_name)==1) {
			skill_name=trim(skill_name, TRIM_BOTH);
...
//		write( "你目前学过：\n\n");		
		...
		if (mappingp(learned_skill_points))
		{
