...
int perform_1(object me, object target)
{
    ...
    if ( duration >0) {
		tell_object(me,"你还需要等待"+(duration/60)+"分钟左右才能再次发动一字长蛇阵。\n");
 		return 1;
 	}
...
"滚滚烟云中，一人黑衣、黑马、黑旗，卷至$N身前叫道：阵法已动，将军速退，待末将断后！\n\n"NOR; 	
  	...
  	seteuid(getuid());
	soldier= clone_object("/obj/npc/danei_guard");
...
"在下$n愿在帐前效力，卫国杀敌，万死不辞。\n\n"NOR; 	
	...
	seteuid(getuid());
	recruit=clone_object("/obj/npc/huwei");
...
"『朝天阙！』"HIY"阵后转出一将，"
	...
	huwei = filter(env, (: living($1) && $1->query("possessed")== $2
