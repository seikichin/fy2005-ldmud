...
int main(object me, string arg)
{
	...
	if( !victim->is_character())
		return notify_fail("你可用get 东西 from 容器。\n");
	if (victim->is_corpse())
	...
	if (!COMBAT_D->legitimate_kill(me, victim)) {
		...
		if (wizardp(me)) tell_object(me,"sp="+sp+", dp="+dp+"\n");
        me->set_temp("stealing", 1);
        call_out( "compelete_steal", 3, me, victim, ob, sp, dp);
        return 1;
}

static void compelete_steal(object me, object victim, object ob, int sp, int dp)
{
