...
struct coordinate *newpath = ({});/*待处理队列*/
struct coordinate enter;/* 入口坐标 */
struct coordinate leave;/* 出口坐标 */
...
protected string mark_ob;				// who got the "maze" mark?
/******************* ---- END ---- *********************/
...
// 重新创建迷宫
void refresh_maze();
void remove_all_players(mixed maze_room,mixed exile_room,string remove_msg);
...
{
	int i,j;
	struct coordinate *valid_leaves=({}),temp;
...
*/
	remove_all_players(this_object(),link_entry_room,"突然传来一阵天崩地裂般的撼动，你吓得连忙冲出迷宫，\n转头看去，整个迷宫已经崩毁於无形。\n\n\n");
	remove();
		...
		for(y=0;y<height;y++)
		{
			for(z=0;z<altitude;z++)
			{
				...
				if(objectp(m_room = find_object(c))) {
					...
					num=sizeof(inv);
					while(num--) {
						if (userp(inv[num]) || inv[num]->is_corpse() && inv[num]->query("player")) 
							inv[num]->move(e_room);	
...
//		return 0;
//	CHANNEL_D->do_sys_channel( "sys", sprintf("file is %s", str));
	...
	{     
		...
		if (o->query("private_maze/maze"))
			ob=o->query("private_maze/maze");
		else
		{
			ob=clone_object(__DIR__"standardmaze");
			ob->set("virtual_room",1);
			o->set("private_maze/maze",ob);
...
//		log_file("debug.log", sprintf("\n ~ %O ~\n", ob));
	...
	if(!maze_built)	// 迷宫未建立
	{
		o1 = find_player(owner);
		o1->set("private_maze/maze",this_object());
		mark_ob = o1->query("id");
	...
	o1 = find_player(owner);
	if(!o1->query("private_maze/maze")) o1->set("private_maze/maze",this_object());
