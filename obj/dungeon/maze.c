...
void remove_all_players(object maze_room,object exile_room,string remove_msg);
...
*/
	remove_all_players(this_object(),this_object(),"突然传来一阵天崩地裂般的撼动，你吓得连忙冲出迷宫，\n转头看去，整个迷宫已经崩毁於无形。\n");
	remove();
...
//	log_file("debug.log", sprintf("\ncontinue..\n", str));
	...
	else if (req_owner != owner)	// request owner not this owner
	{
		...
		if (o->query("private_mazex/maze"))
			ob=o->query("private_mazex/maze");
		else
		{
			ob=clone_object(__DIR__"maze");
			o->set("private_mazex/maze",ob);
		}
	...
	if(!maze_built)	// 迷宫未建立
	{
		this_player()->set("private_mazex/maze",this_object());
		mark_ob = this_player()->query("id");
	//	owner=mark_ob;
	//	create_maze();
		limited_eval((: create_maze() :),0);
	}
