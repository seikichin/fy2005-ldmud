...
void create() {
 //  _debug(1);
  if (clonep()) {
    if (!PO || efun::object_name(PO) != SOCKETD) {
      destruct(ME);
      return;
    }
    move_object(ME, PO);
...
object so_init(mapping socks, mapping usedfd, int fd) {
  if (!clonep()) return 0;
  if (efun::object_name(PO) != SOCKETD) return 0;
...
public object New(int type, mixed host, int port, closure cb, int opts) {
  ...
  while (1) {
    so  = ({object}) clone_object(program_name(this_object()))->so_init(sockets, used_fd, fd);
    ufd = UNIQUE_ID(so);
...
static void lookup(string host, closure cb) {
  ...
  send_erq(ERQ_LOOKUP, to_array(host) + ({ 0 }), cb);
}
...
void so_get_mudmode(int* data) {
  ...
  string err;
  int i;
  if (efun::object_name(PO) != SOCKETD)
...
int remove() {
  ...
  if (efun::object_name(PO) != __MASTER_OBJECT__ && !SPECIAL_PREVILEGE) {
    printf("DONT DO THAT!\n");
    return 0;
  }
...
public int netstat(string arg) {
  ...
  foreach (fd : sort_array(m_indices(sockets),#'>)) {
    ...
    printf("%2d  %-10s %-7s %-21s %-7s %-11s %c %-10s\n",
      ...
      sockets[fd, S_OWNER] + regreplace(efun::object_name(sockets[fd, S_OBJECT]),
      ".*/([^/#]+).*", ":\\1", 0));
  }
