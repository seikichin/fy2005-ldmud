...
"的[37m法杖[32m没顶而插\n");
  ...
  set_temp("apply/dodge", 200);
	...
	setup();
  	...
  	if (!query("claw_set")) {
			if (!random(5)){
				staff=clone_object(__DIR__"obj/wolf-fist");
...
void die(){
  ...
  if (me) {
	    ...
	    if (sizeof(ppl)) {	    
       		ppl_present=filter(ppl,(:objectp($1) && userp($1) && environment($1)==environment():));
	    }
	...
	staff=clone_object(__DIR__"obj/tigersoul");
