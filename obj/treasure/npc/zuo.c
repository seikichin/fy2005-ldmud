...
void die()
{
	...
	if (random(2)) {
		lotus = clone_object(AREA_XINJIANG"obj/lotus");
...
void smart_fight()
{
	...
	case 0:		
		if (!present("定",environment()))
		{
			...
			target = clone_object(__DIR__"obj/ward2");
	...
	case 1:
		if (!present("伤",environment()))
		{
			...
			target = clone_object(__DIR__"obj/ward3");
			target->move(environment());
			target->heal_ob(me,10);
			return;
		}
		...
		&& (query("kee") < query("max_kee") / 3 || query("gin") < query("max_gin") / 3 || query("sen") < query("max_sen") / 3))
	{
		...
		target = clone_object(__DIR__"obj/ward");
		target->move(environment());
