...
int do_mail(string arg) {
	...
	if(({int})this_player()->query("mail/last_sent") + 60 > time()) 
		return notify_fail("系统正在处理你的邮件，请稍后再寄信。\n");
	...
	write("信件标题：");
	input_to("get_mail_title",INPUT_PROMPT,"",mail);
	return 1;
}
...
void get_mail_title(string str, mapping mail) {
	...
	write("信件内容：\n");
	this_player()->edit( (: get_mail_text(mail,$1):) );
}

void get_mail_text(mapping mail, string str) {
	mail["text"] = str;
	write("您自己要留一份备份吗(y/n)？[n]");
	input_to("confirm_copy", INPUT_PROMPT,"",mail);
}
