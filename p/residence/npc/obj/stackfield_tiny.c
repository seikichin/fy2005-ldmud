...
void create()
{
	set_name("杂物架", ({ "stack field"}) );	// 让这个id有别于其他东西。不使用单字id!
	set_weight(30000);
	set_max_encumbrance(8000000);
	set("unit", "个");
	set("prep","in");
	set("desc", "\n"
"一个用来堆各种杂物的架子，一般来说，如果没有天灾人祸，\n"
"这里面的物品只有当系统重启后才会消失。（容量30，当然，\n"
"一些原本不能放入容器的物品是无法放在架子上的）\n");
	set("no_get",1);
	set("no_sell",1);
	set("no_drop",1);
//	set("outdoors",1);
...
int do_dismantle(string arg)
{
	...
	if( !mappingp(item_list) || !sizeof(item_list) )
...
//		set("long",query("desc")+"仔细看去，隐约可见几个小字：江门铸造。\n");
		...
		container_size = MAX_ITEM;		// quest(size) is conflict here now.... !!!!!!
		write(sprintf("这里面有：（%d/%d）\n",sizeof(inv),container_size));
	...
	if(sscanf(item, "%d %s", amount, item)==2) {
		...
		if( amount > obj->query_amount() )
			...
			return 1;
				...
				return 1;		// use "1" to stop the normal "put" fuction.
		}
	}
...
int do_put(object me, object obj, object dest)
{
	...
	int container_size;
	mixed *info;
	info = filter(call_out_info(), (: $1[0]==$2 :), obj);
	
    if(({int}) obj->query("no_drop") ||({int}) obj->query("no_get")||obj->query("lore")) {
    	tell_object(me, "此物品无法放在杂物架上。\n");
    	return 0;
    }	
	
	if (obj->is_container()) {
		tell_object(me, "你不能把容器放在另一个容器里。\n");
		return 0;
	}	

	if( obj == dest) {
		tell_object(me, "你不可以这么做。\n");
		return 0;
	}
	
	if(dest->is_closed()) {
		tell_object(me, "你要先把" + dest->name() + "打开。\n");
		return 0;
	}
