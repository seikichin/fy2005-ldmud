...
nosave mapping events = m_allocate(0);
...
{
        mixed event;
       if(member(events,id)) event = events[id];
        if( !member(events,id) ) return 0;
        ...
        if( functionp(event[EVENT_OBJECT]) )
        {
                ...
                apply(#'call_other,event[EVENT_OBJECT], args);
        }
...
varargs int set_event(mixed time, int loop, mixed ob, mixed func, varargs mixed args)
{
        mixed event;
