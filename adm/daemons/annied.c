...
// 處理各種buff的監測輿實行,提供list,提供debuff能力
	...
	天机的五大神功：Quest_atk, quest_dmg, quest_atk, quest_parry, quest_kee
		...
		closure  finish_fuction,	// disa_type == 2: exec this closure  when buffup interrupted and tell, 3 exec w.b.i and m_v.
...
int remove_buffup(object who,int index_num,int flag)
{
	mapping buff;
	object caster;
	string type,name, buff1,buff2,buff3, buff0;
	...
	finish_closure =buff["finish_closure "];

	if(closurep(finish_closure ))
		if ((flag && buff["disa_type"] >= 2) || !flag)	// 非正常中断，则disa_type 2|3时才执行此程式段。
			evaluate(finish_closure ,who,buff["caster"]);
...
mapping lotto_checkout1(string id){
   return  limited_eval((: lotto_checkout(id) :),0);
}
mapping lotto_checkout(string id)
{
	int i;
...
mapping lotto_getlist1(string id){
 return  limited_eval((: lotto_getlist(id) :),0);
}
...
void lotto_roll1(object me){
 return  limited_eval((: lotto_roll(me) :),0);
}
void lotto_roll(object me)
{
	int i;
...
void lotto_add(object me,string arg)
{
//  2:ROUND3~月(mingyue).3-1-2-3-5-4
...
void lotto_swarm_add(object me,string arg)
{
//  2:ROUND3~月(mingyue).3-1-2-3-5-4
	lotto_money++;
	countnum++;
...
void generate_drop_item(object me,object who)
{
		...
		// Gold reward
/*		if (me->query("gold")) {
			gold = clone_object("/obj/money/gold");
		...
		if (fd) {
			for (i=0;i<fd;i++) {
				...
				item = clone_object(list[random(sizeof(list))]);
