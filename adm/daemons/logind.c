...
static void get_passwd(string pass, object ob);
...
void logon(object ob) {
      ...
      configure_interactive(0, IC_ENCODING, "GBK");
        ...
        if( mappingp(tmp_time = blocks[str]) ) {
          ...
          login_usr = filter(children(LOGIN_OB), (: query_ip_number($1) == $2 :), str);
...
private void get_encoding(string arg, object ob) {
	...
	if(arg == "0" || arg == "０") {
        	 configure_interactive(this_interactive(), IC_ENCODING, "GBK");
		ob->set_temp("char_encoding", "gbk");
	} else if(arg == "1" || arg == "１") {
		 configure_interactive(this_interactive(), IC_ENCODING, "UTF-8");
		...
		write("    1) utf编码 (utf-8) \n");
	     configure_interactive(this_interactive(), IC_ENCODING, "GBK");
	      write("   请选择您使用的内码:\n");
		input_to((:get_encoding($1,ob) :),INPUT_PROMPT,"");
		return;
	}
	write("\n\n");
	UPTIME_CMD->main();
...
//max_dummy
	...
	write("您的英文名字：");
	input_to((:get_id($1,ob) :),INPUT_PROMPT,"");
}
...
// toomany guets nowdays
	...
	{
//		write("a"+file_size(ob->query_save_file() + ".o"));
		if( ob->restore() )
		{
			if (wiz_level(arg)) {
				write("此英文名字已被使用，请输入此帐号的密码：\n");
                          input_to((:get_passwd($1,ob):), INPUT_NOECHO);
			} else
			{
...
/*				alle="/feature/nada.c"->my_number();
	...
	usr = filter(children(LOGIN_OB), (: interactive($1) :));
	i = sizeof(usr);
	while (i--)
	...
	input_to((:confirm_id($1,ob):), INPUT_PROMPT,"");
}
...
""
					...
					write("您的中文名字：");
					input_to((:get_name($1,ob):), INPUT_PROMPT,"");
				return;
			}
		}
	}
...
static void get_passwd(string pass, object ob)
{
	...
	if (user)
	{
		...
		input_to((:confirm_relogin($1,ob, user):),INPUT_PROMPT,"");
		return;
		}
	}
...
private void get_resp(string arg, object ob)
{
	if( arg=="" )
	{
		...
		input_to((:get_resp($1,ob):),INPUT_PROMPT,"");
		return;
	}
...
"	[1;31m请设定您的密码：[0;32m\n");
		input_to((:new_password($1,ob):),INPUT_NOECHO);
		return;
	}
...
private void new_password(string pass, object ob)
{
	...
	input_to((:confirm_password($1,ob):),INPUT_NOECHO);
}
...
private void confirm_password(string pass, object ob)
{
	...
	write("您的电子邮件地址：");
		input_to((:get_email($1,ob):),INPUT_PROMPT,"");
}
...
private void get_email(string email, object ob)
{
	...
	input_to((:get_gender($1,ob,user):),INPUT_PROMPT,"");
}
...
//	int n;
	...
	user = clone_object(ob->query("body"));
	if(!user)
	{
	...
	efun::configure_object(user, 2, ob->query("id"));
       efun::configure_object(ob, 2, ob->query("id"));
	return user;
}
...
varargs void enter_world(object ob, object user, int silent)
{
	...
	cloth = clone_object("/obj/armor/cloth");
	if(objectp(cloth)) {
		cloth->move(user);
		cloth->wear();
	}
	cloth = clone_object("/obj/login_item/wineskin");
	if(objectp(cloth)) {
		cloth->move(user);
	}
	cloth = clone_object("/obj/login_item/beefmeat");
	if(objectp(cloth)) {
		cloth->move(user);
	}
		cloth = clone_object("/obj/login_item/player_bag1");
	if(objectp(cloth)) {
		cloth->move(user);
	}
	if(!silent )
	...
	write("请敲回车键［ＲＥＴＵＲＮ］．．．．．\n");
	input_to((:read_msg($1,user):),INPUT_PROMPT,"");
   }
}
...
//      string tempip;
    ...
    if (file_size(tmp)>0)
    {
        ...
        wiz_ips = old_explode(read_file(tmp), "\n");

        if(sizeof(wiz_ips)==0) return 3;
        for(i=0;i<sizeof(wiz_ips);i++)
        {
		if (regmatch(ip, wiz_ips[i]) && wiz_ips[i]!="")
                {
                        sign=1;
                }
        }
...
object find_body(string name)
{
	...
	for(int i=0; i<sizeof(body); i++)
		if( clonep(body[i]) && geteuid(body[i]) == name ) return body[i];
...
{
	if( wiz_level(this_player()) <= level )
		return 0;
...
int cphirdir(string dirstr, string newdirstr) {
	...
	for(i=0; i<sizeof(dir); i++) {
		...
		copy_file(dirstr+"/"+dir[i], newdirstr+"/"+dir[i]);
	}
	return 1;
}
