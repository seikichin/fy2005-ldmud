...
//	gift time
	...
	{
		food = clone_object("/obj/food_item/newbie_food");
		drink = clone_object("/obj/food_item/newbie_drink");
		drug = clone_object("/obj/medicine/newbie_drug"); 
...
void checkrace(object ob)
{
	...
	if (title_race == 1)
	{
		reward = clone_object(f_price);
		reward->move(ob);
		ob->set("title_race", -1);
	}
        if (title_race == 2)
        {
                reward = clone_object(s_price);
                reward->move(ob);
                ob->set("title_race", -2);
        }
        if (title_race == 3)
        {
                reward = clone_object(t_price);
                reward->move(ob);
                ob->set("title_race", -3);
        }
	return;
...
void loadall(object me)
{
	...
	if( sscanf(created[j], "%s.c", name) == 1 )
		{
			thing = clone_object( DATA_DIR + "login/" + id[0..0] + "/" + id + "/" + name + ".c");
			write( thing->name() + "\n");
                        thing->restore();
