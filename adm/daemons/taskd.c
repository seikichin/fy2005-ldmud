...
void create() {
	...
	limited_eval((: auto_reset_quest(100) :),0);
//	check_room();
}
...
int init_dynamic_quest(int hard) {
	...
	i = sizeof(quests);
	while(i--) {
		call_out("spread_quest",i+2,quests[i],hard);
	}
...
// try not to put task items on floor, however, give up after 3 tries
	...
	if (obj0 == cur_obj && tries >=3) {
		...
		boss=clone_object("/obj/npc/task_npc");
		if (!boss) {
		...
		boss->move(obj0);
		tar = clone_object(quest["file_name"]);
		tar->set("value",0);
...
int sort_quest_list(mapping a, mapping b) {
	if(a["done"] == b["done"]) {
		return a["item"]->query("id")>b["item"]->query("id");
	} else {
...
string dyn_quest_list1(){
return limited_eval((: dyn_quest_list() :),0);
}
...
string dyn_quest_list() {
	...
	for(i=0; i<sizeof(quest_list); i++) {
		...
		output += sprintf("%-*s", 45, quest_string);
		if(i%2) {
			output += "\n";
		}
	}
...
string locate_obj(object me,string strr) 
{
	...
	object item,room;
	string output,dis,alt,dir,temp;
...
object *check_ppl_task(object ob){
	...
	quest_item = filter(inv,(: $1->query("dynamic_quest") :));
	return quest_item;
}		
