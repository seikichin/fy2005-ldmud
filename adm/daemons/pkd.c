...
mapping *hunter;					//	巡捕纪录
mapping field = m_allocate(3);
mapping format = m_allocate(3);
...
mapping *read_table(string filename)
{
        ...
        line = old_explode(read_file(filename), "\n");
        data = ({});
...
//        write(sprintf("field=%O\n format= %O \n",field_1,format_1));
        ...
        for( rn = 0, fn = 0; i<sizeof(line); i++) {
                if( line[i]=="" || line[i][0]=='#' ) continue;
                if( !fn ) data += ({ m_allocate(sizeof(field_1)) });
...
"#  rewarded:   缉捕成功次数\n\n"; break;
	...
	field_2 = old_explode( field[filename], ":" );	
