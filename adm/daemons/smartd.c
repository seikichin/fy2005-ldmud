...
//	seteuid(getuid());
	...
	if (sscanf(me->query("skill_type_w"),"%s/%s",weapon,skill)==2) {
		weapon1 = clone_object(weapon_dir+weapon);
		weapon1->move(me);
		weapon1->wield();
	}
	me->reset_action();
...
//	Here we introduce a clone_object set "smart_busy" to seperate busy and attack frequency, 
...
/*				if (!check_health(me,70)) {		// 这个太厉害了，要秒杀的
	...
	case "musician":
		inv = all_inventory(me);
		instr = filter(inv,(: $1->query("timbre") :));
...
object *resort_enemy(object me, string flag) {
	...
	npc_enemy=filter(enemy,(: !userp($1) :));
	pc_enemy=filter(enemy, (: userp($1) :));
...
int check_weapon(object me, string weapon_type) {
	...
	inv=all_inventory(me);
	if (sizeof(inv))
		weapon=filter(inv,(: $1->query("skill_type")==$2 :),weapon_type);
	if (weapon)
...
void sheet_fight(object me) {
        ...
        seteuid(getuid());
        soldier = clone_object("/obj/npc/hell_guard");
...
void sq_guard(object me) {
        ...
        spells = me->query_skill("scratching");
        seteuid(getuid());
        soldier = clone_object("/obj/npc/taoist_guard");
        soldier->move(environment(me));
        soldier->invocation(me, spells);
		soldier->set("possessed", me);
		me->add_temp("max_guard",1);
...
void alchemy_fight(object me, object target, string weapon_name) {
	...
	weapon = clone_object("/obj/specials/alchemy/magic/"+weapon_name);
	weapon->move(me);
