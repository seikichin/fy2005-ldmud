...
/*	******* warning **********
	THERE is NO monster in current FY. this is just a copy of dragon.c
	You need modify all descriptions if you want to implement a monster race.
*/
	...
	if (!userp(ob))
	{
		...
		if (tough)
		{
			if (undefinedp(my["max_kee"]))
				my["max_kee"] = my["max_kee"] * tough /100;
			if (undefinedp(my["max_gin"]))
				my["max_gin"] = my["max_gin"] * tough /100;
			if (undefinedp(my["max_sen"]))
				my["max_sen"] = my["max_sen"] * tough /100;
		}	} 	
