...
//	These information shouldn't be changed anymore.	
	...
	my_file= replace_string(my_file,"LONG_TITLE",replace_string(save_value(wrap(l_title)),"#3:2\n",""));
...
void process()
{
	...
	while (i--)
	{
		...
		{
			// read .c files.
			room = find_object(dir+file[i]);
			if (!room)
				room = load_object(dir+file[i]);
			room->init();
