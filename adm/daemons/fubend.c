...
public mixed compile_object(string file)
{
       ...
       if(file[<2..]==".c") file=file[0..<3];
        ...
        if( file_size(filename+".c") > 0 ) {
                if( objectp(ob = clone_object( filename )) ) {
        ...
        while( 1 ) {
        	nn=nn+1;
                ...
                if( !maze )catch( maze = load_object(pname);nolog);
                if(  maze ) {
...
mixed compile_private_object(string file)
{
        ...
        ob = clone_object(rfile);
        if( ob ) {
                ob->delete("exits");
                ob->set("exits",([
                        "out" : rfile,
                ]));
                ob->set("valid_startroom",0);
                ob->set("maze",1);
        }
        ob->set_virtual_flag();
        return ob;
...
int enter_fuben_private(object me, string file)
{
        ...
        if( member(fuben_private_list,file) ) {
                ...
                if( time()-me->query("fuben_private/"+fbname) < time ) {
                ...
                if( me->in_team() ) {
                        foreach( object usr in me->query_team() ) {
                                ...
                                if( time()-usr->query("fuben_private/"+fbname) < time ) {
                ...
                owner = me->query("id");
                catch(startroom = load_object("/p/"+me->query("id")+file));
                if( startroom ) {           
                        nfile = fuben_private_npc[file];
                        npc = clone_object(nfile);
                        ...
                        me->move(startroom);  
                        DEBUG_C(me->query("name")+"("+owner+")"+"进入了副本"+fbname+"。");                    
                        ret = SCHEDULE_D->set_event(dest_time, 0, this_object(), "delay_clear_fuben_private", file, owner);
                        me->set_temp("fuben_private_event", ret);
                        me->set("fuben_private/"+fbname, time()); // 记录时间
                        if( !member(fuben_private_all,fbname) )
...
public int enter_fuben(object me, string fbname)
{
        ...
        if( member(fuben_data[fbname],"team") ) {
                if( !me->in_team() ) {
                        ...
                        foreach( ob in inv ) {
                                if( interactive(ob) || base_name(ob)=="/clone/user/user" ) {
                                        if( room->query("exits/out") )
                                                outroom = find_object(room->query("exits/out"));
                                        else
                                                outroom = find_object("/d/fy/fysquare.c");
                                        if( outroom )
                                                ob->move(outroom);
                                        else
                                                ob->move(VOID_OB);
...
public int clear_fuben(string fbname,string owner)
{
        ...
        */
        ...
        if( member(fuben_data[fbname],"leave") ) {
...
public int valid_enter(object usr, string fbname)
{
        ...
        if( fbname == "forest" ) {
                if( member(all,fbname) &&
                    member(all[fbname],query_ip_number(usr)) &&
        ...
        */
        ...
        if( member(fuben_data[fbname],"interval") ) {
                ...
                if( time()-usr->query("fuben/"+fbname) < time ) {
        ...
        if( member(fuben_data[fbname],"single") ) {
