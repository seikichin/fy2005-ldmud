...
void clear_spawn_table(int pos);				// 在 init_spawn_table前清理所有非ｒｏｏｍ的复制物件
void dis_spawn_table(int pos);
...
{
        int i;
        clear_spawn_table(0);		// All previous have to be destructed.
...
//        CHANNEL_D->do_sys_channel("sys",sprintf("初始化分布列表，\n%O\n初始化完毕。\n",spawn_table));
        call_out("dis_spawn_table",SPAWN_INTERVAL,2);	
}

void show_spawn_table()
{
	CHANNEL_D->do_sys_channel( "sys",sprintf("当前物品分布情况为：\n%O\n\n", spawn_table));
}
...
void dis_spawn_table(int pos)
{
	...
	for(i=pos;( i<sizeof(spawn_table)) && (get_eval_cost() >100) ; i++)
	{
		...
		if (spawn_table[i]["counter"] == 0)	
		{
			...
			CHANNEL_D->do_sys_channel("sys",sprintf("系统于%s开始分布%s\n",  "/cmds/usr/uptime"->chinese_time(5,time()+""),name));
	...
	if (i<sizeof(spawn_table))
	{
    call_out("dis_spawn_table", 2,i);
    }
   else
   {
   	call_out("dis_spawn_table",SPAWN_INTERVAL,0);
   }
}
...
void clear_spawn_table(int pos)
{
	int i, j;
	string name;
	object *list;
	
	for(i=pos;(i<sizeof(spawn_table)) && (get_eval_cost() > 150000) ;i++)
	{
		if (spawn_table[i]["type"] == "room")	continue;
...
//             	"sys",sprintf("系统清除%O @ %O。\n",list[j], environment(list[j])));
         ...
         if (i<sizeof(spawn_table))
         {
         call_out("clear_spawn_table", 3,i);
        }
	return;	
}
