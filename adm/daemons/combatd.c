...
// This closure  calculates the combined skill/combat_exp power of a certain
// skill. This value is used for A/(A+B) probability use.
int skill_power(object ob, int usage) {
	int statuss, level, enc, max_enc, result;
	int bonus, exp;
...
// DEBUG OFF		
	...
	if((statuss = ob->query("kee")) > 0 ) {			// Kee will affect SKILL_power
		if(statuss<ob->query("max_kee"))
		result = statuss*100/ob->query("max_kee")*result/100;
	} else {
		result = result /4000;
	}
	...
	if(ob->query("sen") > 0 && ob->query("gin") > 0) {	// Sen will affect EXP_power
		statuss= 3 * ob->query("sen") + ob->query("gin");
		if(statuss< 3*ob->query("max_sen") + ob->query("max_gin"))
			exp = statuss*100/ (3*ob->query("max_sen")+ob->query("max_gin"))*exp/100;
	} else {
		exp = exp /4000;
	}
...
int attack_damage(object attacker, mapping action, object victim) {
	...
	if (!attacker->query_temp("marks/r1_pfm")) {
		...
		if(force_factor && (force_skill = attacker->query_skill_mapped("force"))) {
			tmp = SKILL_D(force_skill)->hit_ob(attacker, victim, d1+d2+d3);
			if(stringp(tmp) ) {
				damage_message += tmp;
			} else if(intp(tmp)) {
				d4 += tmp;
...
// dodge_attack
// the closure  that checks if the attack can be dodged.
...
//	Quest buff-atk
	...
	if (userp(victim)) {
		...
		m_enemy = filter(m_enemy,(: $1->query("combat_exp")>= $2 :), v_exp);
		num = sizeof(m_enemy);
		if (num >= 2) {
...
// parry_attack
// the closure  that checks if the atack can be parried.
int parry_attack(object attacker, int ap, mapping action, object victim) {
	...
	object weapon, armor, att_weapon;
	object* m_enemy;
	string wptype,pski;
	closure  f;
...
// counter_attack
// the closure  checks if we can counter the attack.
...
// do_protect()
// a closure  that checks to see if there is anything/anyone protects our victim
...
// do_attack()
// Perform an attack action. This closure  is called by fight() or as an
...
//		ap = ap*2/3;
	...
	// Disabled now to save statuss report  6/12/2006
/*	if(userp(attacker)) {
...
/*			if(userp(victim)){
	...
	// (9) Prepare final result and statuss message, and show it.
	...
	if(victim_damage) {
		...
		//加到这里，造成了damage，才会中断closure  busy
		if(victim->is_busy())
...
// since most spell/magic perform don't call combatd fight(), this closure  needs to
...
// auto_fight()
// This closure  is to start an automatically fight. Currently this is
...
// This closure  is to announce the special events of the combat.
...
//	This closure  is to modify all KILL closure . All skills/perform that involving "kill_ob" without "kill"
//	should call this closure  to check legitimacy. 
...
//	This closure  is to create a more complicated busy/anti-busy situation, 
...
/*	Currently this closure  does nothing.
	However, all damages that don't directly call do_attack() should go through this closure .
	Later we could
...
//	int i;
	...
	{
		ppl = filter(all_inventory(room),(:($1->query("env/no_others_cbm")==1 || $1->query("env/no_cbm")==1)
			 && $1!=$2 && $1!=$3:), you, me);
		ppl += ({ me });
		ppl += ({ you});
