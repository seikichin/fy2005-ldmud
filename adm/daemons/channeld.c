...
static mapping channels = ([
    ...
    "teamtalk":(["name"     :"队伍",
              "msg_speak": WHT"【%s】%s：%s\n" NOR,
              "msg_emote": WHT"【%s】%s\n" NOR,
              "listeners": (:team_listener($1):) ]),
    "bangtalk":(["name"     :"帮会", //will be changed
              "msg_speak": HIW"【%s】%s：%s\n" NOR,
              "msg_emote": HIW"【%s】%s\n" NOR,
              "listeners": (:bang_listener($1):),
              "flag"     : C_LOG ]),
	"orgtalk":(["name"     :"组织", //will be changed
				"msg_speak": YEL"【%s】%s：%s\n" NOR,
				"msg_emote": YEL"【%s】%s\n" NOR,
				"listeners": (:org_listener($1):) ]),
    ]);
...
int do_sys_channel(string verb, string arg)
{
	...
	obs = filter(users(),"filter_listener",this_object(), channel);
                       ...
                       int cemote)
{
	...
	if (interactive(me) && !wizardp(me))
	{
		...
		if (strlen(arg) >= 160 || strlen(verb) >= 160)
	...
	else //It's a public channel (verb is mapped in channels)
	{
		...
		if (userp(me)) 
		{
			...
			// All public talks are recorded
				log_file(LOG_FILE,sprintf("%O 『%O 』%O (%O )：%O \n",
		...
		obs=filter(obs,"filter_listener",this_object(), channel);
		...
		if(isemote) {			
			...
			if(!undefinedp(emote[emverb])) { //it is a public emote
				if(stringp(emarg)) { //there is a target
					...
					if(objectp(target) && me->visiblep(target)) {
						target_gender = target->query("gender");
						target_name = target->query("name");
					} else {
...
//	}
      ...
      "channel" : verb,
      "talker":me->name(1),
      "text":wrap(remove_ansi(msg),48),]);
 //   printf("channel_info:%O\n",channel_info);
      me->process_gmcp(channel_info,"Comm.Channel","Text");
	return 1;
}
