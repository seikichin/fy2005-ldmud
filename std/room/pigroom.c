...
object server;
string claimer;
mapping	pl = m_allocate(4),
 	cards = m_allocate(4),
	card_count = m_allocate(4),
	roundcards = m_allocate(4),
	picks = m_allocate(4),
	pick_count = m_allocate(4),
	passed = m_allocate(4),
	agreed = m_allocate(4),
	hscore = m_allocate(4),
	tscore = m_allocate(4);
...
void do_assign_cards()	// assign cards to players
{
	...
	for (i = 0; i < 4; i++) {
		cards[seat[i]] = sort_array(cards[seat[i]], #'>);
