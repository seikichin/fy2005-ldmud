...
//  from /d/place/player/clone_objectroom.c                                        //
...
void clone_obj(string line,string datafile,string file)
{
    ...
    {
        obj=clone_object(VOBJ);//需要virtual object吗？
        obj->restore_me();
...
void reset()
{
    ...
    for(i=sizeof(inv)-1; i>=0; i--)
	{
        ...
        {
            if( ({string}) inv[i]->query("born_from") ==  file_name(this_object()) )
                destruct(inv[i]);
            continue;
        }
    ...
    restore_me(file_name(this_object()));
