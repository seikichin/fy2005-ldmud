...
void init()
{
  ...
  if ( this_player()!=this_interactive() ||	geteuid(previous_object(1))!=geteuid(this_player()))
	return 0;
...
int do_exercise(string arg)
{
	...
	if(!is_owner(me->query("id"))){
		 notify_fail ("这么大的事儿，要主人才能作主。\n");
		 return 0;}
	
	duration = query_temp("fight_switch")+ 3600 - time();
	if (duration >0){
		 notify_fail("改变房屋打斗属性每小时最多一次（"+ (duration/60)+"）\n");
		 return 0;
		 }	
	...
	if (arg == "0") {
	 ...
	 notify_fail("参数只能是１（允许打斗）或０（不允许打斗）\n");
	 return 0;
...
int do_payoff()
{
	...
	if (query("basic_tax") == -1){
		 notify_fail("你的屋子已经交清存款了。\n");
		  return 0;
		  }
	
	if (!objectp(contract = present("payoff contract",me))){
		 notify_fail("为防止误操作，请先去购买一份合约（payoff contract）\n");
		  return 0;
		 }
	...
	gold  = this_player()->query("deposit")/100;
	if(gold < t){
       	 notify_fail("你在南宫家没有存够"+chinese_number(t)+"两银子。\n");
       	 return 0;
       	}
	
	if(me->query("score") < s){
       	 notify_fail("你的评价不足"+chinese_number(s)+"点！\n");
       	 return 0;
       	}
...
int do_remodel(string arg){
	...
	int old_space, clone_object_space, num;
	...
	if(!is_owner(me->query("id"))){
		notify_fail ("这事儿只有主人才能做决定。\n");
		 return 0;
		}
	
	if (query("basic_tax") != -1){
		notify_fail("先把这间屋子的贷款交清再说吧。\n");
		 return 0;
		}
	
	if (!objectp(contract = present("remodel contract",me))){
		 notify_fail("为防止误操作，请先去购买一份合约（remodel contract）\n");
		 return 0;
		}
		
	if (old_space>=5){
		 notify_fail("你的屋子已经具有最大空间了。\n");
		 return 0;
		}
		
	if (!arg || sscanf(arg,"%d",clone_object_space)!=1){
		 notify_fail("格式：remodel 数字，数字为你想扩建到的空间。\n");
		 return 0;
		}
		
	if ( clone_object_space <= old_space || clone_object_space >5){
		 notify_fail("Remodel后的数字必须比你现有空间（"+old_space+"）大，比６小。\n");	 
		  return 0;
		 }
		
	switch (clone_object_space){
...
//	write ("remodel needs " + g + " gold, "+ s + " score \n");
	...
	if(gold < g){
       	 notify_fail("你在南宫家没有存够"+chinese_number(g)+"两金子。\n");
       	  return 0;
       	 }
	...
	env->set("free_space",clone_object_space-(old_space-env->query("free_space")));
	env->save();
...
int do_share(string arg)
{
	...
	if(({string})me->query("id") != query("owner"))
		return 0;
	if (!me->query("marry") ){
		 notify_fail("你只能与你的另一半分享房间。\n");
		  return 0;
		 }
	if (!arg){
		 notify_fail("设置指令为：share id。\n");
		 return 0;
		}
	...
	if (arg == query("owner")){
		 notify_fail("将自己设置为第二个主人？好主意．．．\n");
		 return 0;
		}
	if (query("couple")){
		 notify_fail("你只能设置一次房间的第二主人！\n");
		 return 0;
		}
	if (!find_player(arg)){
		 notify_fail("第二主人必须在其在线时设置。\n");
		 return 0;
		}
	set("couple",arg);
...
string query_save_file()
{
        ...
        return sprintf(DATA_DIR "user/%c/%s/%s", id[0],id, id+".room");
}
int save() {
	string file;
...
/*
	...
	for(i=0; i<sizeof(lists); i++)
	{
		...
		item = clone_object(mitem["item_basename"]);
...
int check_legal_long(string name)
{
        int i;
        i = sizeof(name);
        if( (text_width(name) < 60) || (text_width(name) > 300 ) ) {
...
int input_description(){
	...
	if (env->query("renovation_time")+ 600> time()){
		 notify_fail("装修屋子的间隔时间不能少于十分钟。\n");
		 return 0;
		 }
		
	if (me->query("deposit")< 50000){
		notify_fail("你的银行存款不够，至少需要五两黄金。\n");
		return 0;
		}
		
	me->edit( (:enter_desc( me, env,$1):) );
	return 1;
}
