...
int chat()
{
	...
	if(!this_object()->is_fighting() )
	{
		...
		if (random(100)>50)
		{
			if (random(100)>95|| !this_object()->random_move())
			{
				...
				clone_object_room=TASK_D->random_room();
				if(!room=find_object(clone_object_room))
					room=load_object(clone_object_room);
	...
	if( pointerp(msg = query("chat_msg"))) {
...
void die()
{
	...
	switch (query("quality"))
	{
		case "evil":
			...
			objReward = clone_object("/questnpc/obj/dragon-card");		
			break;
		case "good":
			objReward = clone_object("/questnpc/obj/shanliu-scroll");
