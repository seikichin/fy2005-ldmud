...
int do_project(string arg)
{
	...
	if(!arg) { notify_fail("工作计画请指定一个标题。\n");return 0;}
	...
	note["time"] = time();
	this_player()->edit( (:done_describe_project(this_player(), note,$1) :) );
	return 1;
}
...
int do_report(string arg)
{
	...
	if(!arg || sscanf(arg, "%d %s", num, title)!=2 ){
		 notify_fail("指令格式：report 计画编号 进度报告标题\n");
		 return 0;
		 }

	notes = query("notes");
	if( !pointerp(notes) || num < 1 || num > sizeof(notes) ){
		notify_fail("没有这个工作计画。\n");
		return 0;}
	...
	note["author"] = this_player()->name(1);
	note["time"] = time();
	this_player()->edit( (:done_report( this_player(), note, num-1,$1):) );
	return 1;
}
...
int do_read(string arg)
{
	...
	if( !pointerp(notes) || !sizeof(notes) ){
		 notify_fail("进度表上目前没有任何计画。\n");
		 return 0;}

	if( !arg ) { notify_fail("指令格式：read 计画编号[.报告编号]|new|next\n");return 0;}
	if( arg=="new" || arg=="next" ) {
		for(num = 1; num<=sizeof(notes); num++)
			if( notes[num-1]["time"] > last_read_time ) break;
	} else if( sscanf(arg, "%d.%d", num, rep)==2 ) {
		if( num < 1 || num > sizeof(notes) ) { notify_fail("没有这个计画。\n");return 0;}
		else num--;
		if( rep < 1 || rep > sizeof(notes[num]["report"]) )	{ notify_fail("没有这个进度报告。\n");return 0;}
		else rep --;
...
int do_terminate(string arg)
{
	...
	if( !pointerp(notes) || num < 1 || num > sizeof(notes) ){
		 ...
		 return 0;}
	num--;
	if( notes[num]["owner"] != ({string})this_player()->query("id")
	&&	({string})SECURITY_D->get_status(this_player()) != "(admin)" ){
		 notify_fail("这个留言不是你写的。\n");
		 return 0;}
