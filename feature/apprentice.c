...
int is_apprentice_of(object ob)
{
    	...
    	if( !mappingp(family = query("family")) ) return 0;
...
varargs void assign_title(string title, int nogen)
{
    	mapping family;
    	int gen;
    	if( !mappingp(family = query("family")) ) return;
...
int is_member_of(object ob)
{
    	...
    	if( !mappingp(organization = query("organization")) ) return 0;
...
string org_rank_title(object member)
{
    	...
    	if(!pointerp(rank_levels))
        	rank_levels= ({1000000,5000000,10000000});
    
    	if(!pointerp(ranks) || sizeof(rank_levels)+1 != sizeof(ranks))
    	{
...
{
     	mapping organization;
    	if( !mappingp(organization = query("organization")) ) return;
...
int enroll_member(object ob)
{
    	...
    	if( !mappingp(my_organization = query("organization")) ) return 0;
    	organization = m_allocate(sizeof(my_organization));
