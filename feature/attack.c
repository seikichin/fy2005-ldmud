...
// This closure returns 1 if we are fighting anyone (or with ob)
...
//This closure starts fight between this_object() and ob
...
// This closure starts killing between this_object() and ob
...
/* This closure checks if the current opponent is available or
...
object select_opponent() {
	...
	if (!userp(this_object())) {
		...
		// 实在不行就随便选了
		enemy_weak = filter(enemy, (: $1->query("kee") < $1->query("max_kee")/5 :));
...
// reset_action()
// This closure serves as a interface of F_ATTACK and wielded, worn objects.
	...
	if(userp(me) || userp(ob) || !me->query_temp("is_unconcious")) {
		...
		if(userp(ob) && is_killing(ob->query("id"))) {
			mark = "hatred";
				...
				&& ob->query("vendetta/" + vendetta_mark) && me->visiblep(ob)) {
			mark = "vendetta";
		} else if(userp(ob) && ({string})query("attitude")=="aggressive" && me->visiblep(ob)) {
			mark = "aggressive";
