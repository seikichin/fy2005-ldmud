...
mapping transfer_ts(mapping old_ts) {
  int opt;
  if (!previous_object() || getuid(previous_object()) != "Root") return 0;
  if (!old_ts) return ts;
...
void set_noecho(int flag) {
  ...
  if (extern_call() && efun::object_name(previous_object()) != __MASTER_OBJECT__)
    return;
...
static void start_cs(int command, int option)
{
  ...
  {
    send(({ IAC, SB, TELOPT_CHARSET, CS_REQUEST }) + (to_array("GBK UTF-8") - ({ 0 })) + ({ IAC, SE }));
...
private void sb_naws(int command, int option, int* optargs) {
  ...
  if(!pointerp(old) || old[0] != cols || old[1] != lines) {
  	  //receive_message(string msg,string msgclass)
//    this_object()->receive_message(sprintf("window_size height:%d x width:%d\n", lines, cols),CMSG_GENERIC);
//    this_object()->set("PageWidth", cols);
//    this_object()->set("PageSize", lines-2);
  }
...
static void sb_charset(int command, int option, int* optargs)
{
    ...
    switch (optargs[0])
    {
        case CS_REQUEST:
        {
            ...
            icharsets = map(charsets, (: lower_case(to_text($1, "gbk")) :));
#else
            ...
            // We prefer UTF-8
            if ((pos = member(icharsets, "gbk")) >= 0)
            {
                ts[TS_EXTRA, TSE_ENCODING] = "gbk";
...
private void start_mssp(int command, int option)
{
    ...
    if (sizeof("mail@seikitin.com"))
    send_mssp_var("CONTACT", "mail@seikitin.com");
    send_mssp_var("LANGUAGE", "Chinese");
    send_mssp_var("CODEBASE", "FY");
    send_mssp_var("FAMILY", "LPMud");
    send_mssp_var("INTERMUD", "I2");
    send_mssp_var("INTERMUD", "I3");
    send_mssp_var("CREATED", "2020");
    send_mssp_var("LOCATION", "China");
...
static void ping_no_answer(object ob)
{
  msg_write(CMSG_GENERIC,
    ...
    (ob != this_object()) ? capitalize(({string})ob->name()) : "you"
  ));
}
...
nomask int ping_command(string str)
{
  object ob;
  str = trim(str||"");
  if (!sizeof(str) || !wizardp(this_player()))
    ob = this_player();
  else
  {
