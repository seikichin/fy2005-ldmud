...
// This closure is called by heart_beat to update "continously active"
...
nomask int update_condition()
{
	...
	if( !mappingp(conditions) || !(i=sizeof(conditions)) ) return 0;
	cnd = keys(conditions);
	update_flag = 0;
	while(i--) {
...
//
// This closure applies the specific condition to the character. Note we 
...
//
// This closure returns info about the specific condition if any.

nomask mixed query_condition(string cnd)
{
	if( !mappingp(conditions) || undefinedp(conditions[cnd]) ) return 0;
	return conditions[cnd];
}

// query_all_condition()	查询所有condition。
//
// This closure returns all conditons
...
//This closure removes all general type conditions		
...
nomask void clear_condition()
{
	...
	if( !mappingp(conditions) || !(i=sizeof(conditions)) ) return ;
	cnd = keys(conditions);
...
//This closure clears all general conditions, plus different level
...
//This closure removes all condition what-so-ever
//-neon
