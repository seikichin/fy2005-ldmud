...
void set_items(mapping t_items)	{		// this closure is for transfering
...
string nrstr(string str,int lng,string prefix,string surfix,int ns)
{
        ...
        string strc;
        string * epl;
 epl= old_explode(break_string(str,lng),"\n");
 strc="";
...
//	string tmp1,tmp2;
    ...
    if( (!list || !mappingp(list)) && (!tlist || !mappingp(tlist)))
...
object team_check(object who,string what,int flag,mixed dat)
{
	...
	if(team)
	{
		for (i=0; i<sizeof(team); i++)
		{
			...
			if ( (flag == 0 || flag == 2) 
				&&	(environment(team[i]) != environment(me) || ( !me->visiblep(team[i])) )) 
				continue;
...
int check_legitimate_state(object me)
{
	...
	{
log_file("check_legitimate_state",sprintf("pl: %O str: %O\n",me,me->query("str")));
		block_account(me);
		return 0;
	}
	...
	{
log_file("check_legitimate_state",sprintf("pl: %O int: %O\n",me,me->query("int")));
		block_account(me);
		return 0;
	}
	...
	{
log_file("check_legitimate_state",sprintf("pl: %O dur,agi,con,cps,kar,fle,spi.\n",me));
		block_account(me);
		return 0;
	}
	...
	{
		log_file("check_legitimate_state",sprintf("pl: %O force_factor\n",me));
		block_account(me);
		return 0;
	}
	...
	{
log_file("check_legitimate_state",sprintf("pl: %O combat_exp\n",me));
		block_account(me);
		return 0;
	}
	...
	{
log_file("check_legitimate_state",sprintf("pl: %O get_highest_skill_level\n",me));
		block_account(me);
		return 0;
	}
	...
	{
log_file("check_legitimate_state",sprintf("pl: %O Max gin/kee/sen\n",me));
		return 0;
	}
