...
int wear() {
	...
	if(!mappingp(armor_prop = query("armor_prop"))) {
	...
	if(!mappingp(applied_prop_real)) {
		applied_prop_real = ([]);
	}
	
	applied_prop = owner->query_temp("apply");
	if(!mappingp(applied_prop)) {
		applied_prop = ([]);
	}
...
int wield() {
	...
	if(!mappingp(weapon_prop = query("weapon_prop"))) {
...
//	int i;
	...
	exclude=filter(all_inventory(environment(me)),(: intp($1->query("env/"+$2)) && $1->query("env/"+$2)>1 :),flag);
