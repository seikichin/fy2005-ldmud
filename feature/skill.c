...
void set_skill(string skill, int val)
{
    	...
    	if( !mappingp(skills) ) skills = ([ skill: val ]);
...
int delete_skill(string skill)
{
    	...
    	if( mappingp(skills) ) map_delete(skills, skill);
    	if( mappingp(learned) ) map_delete(learned, skill);
    	if( mappingp(skill_map) )
    	{
        	int i;
        	string *name;
        	name = keys(skill_map);
...
// This closure 'map' a skill <skill> to another skill <mapped_to>.
// If 2nd argument not given, cancel the mapped skill <skill>.
varargs void map_skill(string skill, string mapped_to)
{
    	if( !mapped_to && mappingp(skill_map) ) {
    	...
    	if( !mappingp(skills) || undefinedp(skills[mapped_to]) )
        	return;
		
    	if( !mappingp(skill_map) ) skill_map = ([ skill: mapped_to ]);
    	else skill_map[skill] = mapped_to;
}

string query_skill_mapped(string skill) {
    	if(mappingp(skill_map) && !undefinedp(skill_map[skill]))
        	return skill_map[skill];
    	return 0;
}


//	This closure query real skill level:
varargs int query_skill(string skill, int raw) {
	if(!raw) {
		...
		if(mappingp(skills)) {
			...
			if(mappingp(skill_map) && skill_map[skill]) {
	...
	if(mappingp(skills) && !undefinedp(skills[skill])) {
		return skills[skill];
	}
	return 0;
}

//	This closure query modifiers on skill
...
//	This closure query the combined/final level of skills
...
int skill_death_penalty(int combat_exp) {
	...
	if(!mappingp(skills) || combat_level["level"] < 10) {
		return 0;
	}
...
/*    	spi = this_object()->query_int();
    	...
    	if( !mappingp(learned) ) learned = ([ skill : amount ]);
