...
{
	if( !new_busy ) return;
	if( !intp(new_busy) && !closurep(new_busy) )
		error("action: Invalid busy action type.\n");
	busy = new_busy;
	if( !intp(new_interrupt) && !closurep(new_interrupt) )
...
void interrupt_me(object who, string how)
{
	...
	} else if( closurep(interrupt) ) {
		if( apply(interrupt, this_object(), who, how) ) {
			busy = 0;
			interrupt = 0;
		}
	}
...
// This closure is for temporary conditions's recovery call_outs, bcz
// such recovery closure call_out might be destroyed if some wizard
...
int start_call_out(closure fun, int delay)
{
/*
	if( !previous_object()
...
void eval_function(closure fun) { apply(fun); }
