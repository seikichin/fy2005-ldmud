...
nomask int process_input_do(string verb, string args)
{
 ...
 //       DEBUG_CHANNEL("process_input_do ");
        if( args )
        {
                ...
                foreach( string action in old_explode(args, ",") )
                {
                        do_times = 1;
...
*/
	...
	if( str[0]=='!' ) {
		if( pointerp(history) && sizeof(history) ) {
			if( sscanf(str, "!%d", i) )
				str = history[(HISTORY_BUFFER_SIZE + last_cmd - i) % HISTORY_BUFFER_SIZE];
			else
...
int set_alias(string verb, string replace)
{
	if( !replace ) {
		...
		if( !mappingp(alias) ) alias = ([ verb:replace ]);
