...
string get_flower()
{
	...
	mapping goods;
	string *names;
	if( !mappingp(goods = query("vendor_goods")) ) 	return "";
	names = keys(goods);
	if (!sizeof(names))
	{
...
mapping query_delivery(object who)
{
	...
	i = sizeof(pick_list);
	delivery = old_explode(pick_list[random(i)],":");
	return (["sender": name,
			"receiver": delivery[1],
			"area":		delivery[2],
			]);
...
void test_list(){
	...
	for (i=0;i<sizeof(pick);i++){
		data = old_explode(pick[i],":");
		if (objectp(ob=clone_object(data[1]))){
...
*/
	...
	flower = clone_object(get_flower());
	if (!flower) {
	...
	if (flower->move(who))
	{
		...
		flower->set("delivery_time", time());	
		who->set("delivery_time", time());	
...
int give_deposit()
{
	...
	if (amount>=1)
	{
/*		silver = clone_object("/obj/money/silver");
...
int book_flowers(string arg)
{
	...
	if( mappingp(goods = query("vendor_goods")) ) 
	{
		names = keys(goods);
		if (sizeof(names)) 
		{
