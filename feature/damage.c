...
// This closure is to support Zmud/tintin input information process
...
if (environment() && environment()->alternative_die(this_object())) {
    ...
    if (environment() && closurep(f=environment()->query("alternative_die"))) {
	if (apply(f,this_object())) return;}*/
...
object select_opponent() {
	...
	if (!userp(this_object())) {		
		...
		// 实在不行就随便选了
		enemy_weak = filter(enemy, (: $1->query("kee") < $1->query("max_kee")/10 
					&& userp($1) :));
