...
int perform(object me, object target)
{
	...
	object weapon, shadow, hisweapon, clone_objectweapon;
	object *enemy;
	int cond;
...
"成个圆圈，倏然疾翻，顺势将$n"NOR+YEL"的劲力引向$n。\n" NOR;
	...
	if( (myexp*3/4 + random(myexp)*(4+num) > yourexp || !userp(me)) && !target->query_temp("is_unconcious") ) {
		...
		if( !userp(target) ) {
			...
			shadow = clone_object(temp);
			"/feature/annie"->annie_mimic(target, shadow);
		} else {
			shadow = clone_object("/obj/npc/shadefigure");
			shadow->changeshape(target);
		}
		if( clone_objectweapon = shadow->query_temp("weapon") ) {
			// shadow 自己武器不用考虑
			clone_objectweapon->unequip();
		}
		if( hisweapon = target->query_temp("weapon") ) {
			// 给shadow 加新武器
			sscanf(file_name(hisweapon), "%s#%d", temp1, num1);
			clone_objectweapon = clone_object(temp1);
			clone_objectweapon->move(shadow);
			clone_objectweapon->wield();
		}
		shadow->set("owner", me);
