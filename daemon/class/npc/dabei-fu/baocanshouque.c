...
int perform(object me, object target)
{
    ...
    message_vision( CYN"$N身影一闪，忽然欺近$n身前，柔若无骨的手指轻轻拂上$n的胸膛，一路叩击着下滑到气海丹田。\n" NOR, me,target);
    message_vision( YEL"$n变得面如土色，浑身如筛糠般狂抖，护身的硬功竟在这一瞬间已被$N破去。\n" NOR, me,target);

                target->add_temp("buff/no_abs",1);
                me->start_busy(2);
                target->start_call_out( (: call_other( __FILE__, "remove_effect", target,  i) :), 30);
    return 1;
}
