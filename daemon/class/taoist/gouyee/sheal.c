...
int exert(object me, object target, int amount)
{
	...
	if( userp(me) && me->query("scroll/gouyee_heal") < 3 ) {
		return notify_fail("你还没有悟出复苏灵息III，需要找到其密籍才行。\n");
	}
	level = me->query_skill("gouyee", 1);
	if( level < 150 || me->query_skill("necromancy",1) < 180 ) {
		return notify_fail("『复苏灵息Ⅲ』需要150级的谷衣心法和180级的三清道术。\n");
	}
	if( !target ) {
	...
	if( ({int})me->query("gin") <= 600 ) {
		return notify_fail("『复苏灵息Ⅲ』需要600点的精力。\n");
	}
	if( userp(me) ) {
		me->receive_damage("gin", 600);
	}
