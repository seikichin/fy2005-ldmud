...
int scribe(object me, object sheet)
{
	...
	int amn,cost;
		...
		if(skill<60 )
				return notify_fail("此符至少需要60级三清符术！\n");
		...
		newsheet->set_name(CYN +name+ NOR, ({ "tojing sheet" }));      
...
{ 
	...
	if( random(ap + dp) > dp )
	{
		...
		if (!present("jing sheet",me))
		{
			seteuid( geteuid(me));
			obj = clone_object("/obj/item/magic_seal");
			obj->set_name(YEL+name+  NOR, ({ "jing sheet"}));  
			
			obj->set("desc_ext","恢复"+amn+"点精力");
