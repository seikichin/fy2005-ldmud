...
int exert(object me, object target, int amount)
{
	...
	if( lvl < 200) 	return notify_fail("『天护灵烨』需要200级的破邪心经。\n");

	if (me->query("class")!="demon")
		return notify_fail("只有魔教弟子才能运转『天护灵烨』。\n");
	
	if (!(drug1 = present("yushu",me)) || !(drug2 = present("nishi",me)) )
		return notify_fail("使用『天护灵烨』需有玉枢丹、双彩霓石两物为引。\n");
	...
	mapping buff;
		...
		buff =
			([
				...
				"buff3_c":amount,
				"time":duration,
				"buff_msg":HIW"一片浅色霞云在$N头顶收聚，一点点纳进$N天门．．．\n"NOR,
				"warn_msg":WHT"你觉得身体中的充实之感在一点一点的流失．．．\n"NOR,
				"disa_msg":HIR BLK"你打个冷颤，只觉得身子一下子虚弱了许多！\n"NOR,
				"special_func":1,
			]);
		ANNIE_D->buffup(buff);
