...
int exert(object me, object target, int amount)
{
	...
	if( lvl < 100) 	return notify_fail("『力抗山河』需要100级的破邪心经。\n");

	if (me->query("class")!="demon")
		return notify_fail("只有魔教弟子才能运转『力抗山河』。\n");
	
	if (!(drug1 = present("lianhua",me)) || !(drug2 = present("bailu",me)) )
		return notify_fail("使用『力抗山河』需有莲花瓣、白露水两物为引。\n");
...
*/	// hmm...出钱的就是大爷喔...gold gold,想要的尽管来吧
	...
	message_vision(HIW"$N深深吸了口气，运起真气将物引融化，施起『力抗山河』。\n"NOR, me);	
	me->add("force", -800 );
