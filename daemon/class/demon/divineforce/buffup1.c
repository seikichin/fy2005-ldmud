...
int exert(object me, object target, int amount)
{
	...
	if( lvl < 1) 	return notify_fail("『霞起云飞』需要1级的破邪心经。\n");

	if (me->query("class")!="demon")
		return notify_fail("只有魔教弟子才能运转『霞起云飞』。\n");
	
	if (!(drug2 = present("yinge",me)) )
		return notify_fail("使用『霞起云飞』需有银戈草为引。\n");

	if (!drug2->query("is_drug"))	// 假货?
		return notify_fail("使用『霞起云飞』需有银戈草为引。\n");
...
*/	// hmm...出钱的就是大爷喔...gold gold,想要的尽管来吧
	...
	message_vision(HIW"$N深深吸了口气，运起真气将物引融化，施起『霞起云飞』。\n"NOR, me);	
	me->add("force", -100 );
