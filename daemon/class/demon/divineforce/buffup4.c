...
int exert(object me, object target, int amount)
{
	...
	if( lvl < 150) 	return notify_fail("『仪光道霞』需要150级的破邪心经。\n");

	if (me->query("class")!="demon")
		return notify_fail("只有魔教弟子才能运转『仪光道霞』。\n");
	
	if (!(drug1 = present("yushu",me)) || !(drug2 = present("bailu",me)) )
		return notify_fail("使用『仪光道霞』需有白露水、玉枢丹两物为引。\n");
...
*/	// hmm...出钱的就是大爷喔...gold gold,想要的尽管来吧
	...
	message_vision(HIW"$N深深吸了口气，运起真气将物引融化，施起『仪光道霞』。\n"NOR, me);	
	me->add("force", -1300 );
