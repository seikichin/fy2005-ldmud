...
int conjure(object me, object target)
{
	...
	int duration, time;
        ...
        if(me->query_skill("sheolmagic",1) < 170 )
                return notify_fail("『梦魇召唤』需要170级的夜叉通灵法！\n");
	time = 3600;
	if (!userp(me)) time = 60;
	duration = me->query("timer/demon_guardian4")+ time -time();
	if (duration>0)	
	...
	me->set("timer/demon_guardian4",time());
	seteuid(getuid());
	soldier = clone_object("/obj/npc/guardian4");
