...
int conjure(object me, object target)
{
	...
	int duration, time;
        ...
        if(me->query_skill("sheolmagic",1) < 80 )
                return notify_fail("『妖罗召唤』需要80级的夜叉通灵法！\n");

	time = 3600;
	if (!userp(me)) time = 60;
	duration = me->query("timer/demon_guardian2")+ time -time();
	if (duration>0)	
	...
	message_vision("		"BMAG YEL"★妖罗召唤★\n\n"NOR,me);
	me->set("timer/demon_guardian2",time());
	seteuid(getuid());
	soldier = clone_object("/obj/npc/guardian2");
