...
int perform(object me, object target)
{
        ...
        if (duration>0)
        	return notify_fail("『三途往生』耗费体能极大，需要休息"+(duration/60+1)+"分钟。\n");
    	...
    	if (userp(me))	me->add("force", -400);
     	...
     	write("请输入往生咒的对象名字：");
	input_to( (: call_other(__FILE__, "select_target", me,$1 ):));
...
void select_target(object me, string name)
{
	...
	me->set_temp("last_damage_from","念咒过度累死了。\n");
        ...
        if( !ob || !me->visiblep(ob) || userp(ob) || present(ob, room)|| ob->is_ghost()
...
"若问前生事 　今生受者是　 若问后世事　 今生做者是  ！\n" ;
	...
	{
		seteuid(geteuid(me));
		new1 = clone_object(base_name(ob));
