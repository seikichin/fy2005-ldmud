...
#define INIT_FILE "/adm/etc/preload"
...

{
    ...
    if(member((["/obj/simul_efun.c", "/obj/spare_simul_efun.c", "/obj/master.c" ]), base_file))
        return 0;

    return "#include\"/sys/globals.h\"\n";
}
...
string creator_file(string file)
{
	...
	switch(path[0]) {
		case "adm":
			if( file==SIMUL_EFUN_FILE ) return "LDMud";
		...
		default:
			if( this_player() )
				return getuid(this_player());
			else
				return "LDMud";
	}
	return "NOBODY";
}
protected mixed give_uid_to_object(string date, object po)
{
  string creator,pouid;
  if (!stringp(date)||!objectp(po))  return 1;
  if (strstr(date, "/data/") == 0
      || strstr(date, "/log/") == 0
      || strstr(date, "/ftp/") == 0
      )
      return 1;
  if (!(creator=creator_file(date))) return 1;
  
  if (creator==ROOT_UID && po==TO) return ROOT_UID;
  if (!(pouid=geteuid(po))) return 1; // Disallow if no euid in po
  if (pouid==ROOT_UID)
    return ({creator,"NOBODY"});    // root does not create root objects!
  if (creator==pouid || creator=="BACKBONE")
    return pouid;
...
{
 return(give_uid_to_object(object_name, prev));
}

int valid_seteuid(object ob, string neweuid)
{
  if (getuid(ob) == neweuid)
        return 1;
  if (getuid(ob) == ROOT_UID)
       return 1;
//     debug_message(sprintf("\n valid_seteuid :object ob: %O, string neweuid: %O \n",ob,neweuid));
      return 0;
}
...
{
return(give_uid_to_object(new_name, prev));
}
...

{
 ...
 configure_driver(DC_FILESYSTEM_ENCODING, "UTF-8");
  configure_interactive(0, IC_ENCODING, "GBK");
  set_driver_hook(
        H_MOVE_OBJECT0,
 ...
 set_driver_hook(H_CREATE_OB, unbound_lambda( ({ 'x }),
  ...
  set_driver_hook(H_MODIFY_COMMAND, "process_input");
...
{
    configure_interactive(obj, IC_ENCODING, "GBK");
  ({void})obj->net_dead();
  return;
}
...

{
	...
	object daemon;
	if( daemon = find_object("/adm/daemons/virtuald") )
		return daemon->compile_object(filename);
	else
		return 0;
...

{
    ...
    if (!interactive(ob))
	return;
    tell_object(ob, "\u4e00\u9635\u65f6\u7a7a\u7684\u626d\u66f2\u5c06\u4f60\u4f20\u9001\u5230\u53e6\u4e00\u4e2a\u5730\u65b9....\n");
	if (error = catch(move_object(ob, "obj/void"))) {
		write(error);
	    }
}
...
{
    	if( interactive(what) ) {
		tell_object(what, "\u4e00\u9635\u65f6\u7a7a\u7684\u626d\u66f2\u5c06\u4f60\u4f20\u9001\u5230\u53e6\u4e00\u4e2a\u5730\u65b9....\n");
		move_object(what,"obj/void");
	}
	  destruct(what);
}
			      ...
			      int line, mixed* stack, mixed culprit, int caught, int warn)
{
	...
	if(objectp(target))
	 wiz= target->query_wiz_level();

	if(target && (!warn || wiz))
	    tell_object(target, wiz ? err : "You see a flaw in the space-time structure.\n");
	return;
    }
    error=sprintf("\n err: %O \n prg: %O \n curobj: %O\n line:%O\n stack:%O\n culprit:%O \n  caught: %O\n  warn:%O\n",  err,  prg,  curobj,
			       line, stack,  culprit,  caught,  warn);
...
int valid_exec (string name, object ob, object obfrom)
{
 switch("/"+name)
  {
    case "/cmds/adm/_exec.c":
    case "/cmds/usr/quit.c":
    case "/cmds/wiz/update.c":
...

{
    ...
    path = user_path(getuid(this_player())) +".dead_ed_files";
    if (file_size(path) == -1) {
        mkdir(path);
    }
