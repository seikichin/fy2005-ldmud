...
{
	set("short", "夹道");
	set("long","你现在处于一个窄窄的夹道里，通向北面的路被一块巨石(rock)堵死。夹道的\n"
"两侧是三丈高的石墙(wall)。出乎你的意料，石墙上布满了碗大的圆孔(hole)。石\n"
"墙的后面传出“吱吱嘎嘎”的声音(noise)。\n"
""
	);
	set("exits", ([ 
  "south" : __DIR__"gate_castle",
	...
	create_door("south", "door","城门", "north", DOOR_CLOSED);
	set("coor/x",0);
	set("coor/y",0);
	set("coor/z",10);
	setup();
...

{
        ...
        if( environment(ob) == this_object())
	{
		...
		damage = random(10)+35	;
        	ob->set("eff_kee",(i - damage));
		i =  ({int})ob->query("kee");
		ob->set("kee",(i - damage));
		ob->set("last_damage_from","在黑松堡下中乱箭穿身而亡。\n");
		if( environment(ob) == this_object())
