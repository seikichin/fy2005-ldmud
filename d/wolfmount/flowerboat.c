...
void create()
{
	...
	set("long","小小的舟上，堆满了鲜花，各式各样的鲜花，从远山采来的鲜花。风中这花舟\n"
...
void laopistory(object laopi,object npc,object me){
	...
	grass = clone_object(__DIR__"npc/obj/grass");
...
void reset()
{
	...
	{
		item = clone_object(__DIR__"obj/flower1");
		item->move(this_object());
	}
	if (!present("gardenia",this_object()))
	{
		item = clone_object(__DIR__"obj/flower2");
		item->move(this_object());
	}
	if (!present("daisy",this_object()))
	{
		item = clone_object(__DIR__"obj/flower3");
		item->move(this_object());
	}
	if (!present("hyacinth",this_object()))
	{
		item = clone_object(__DIR__"obj/flower5");
		item->move(this_object());
	}
	if (!present("lily",this_object()))
	{
		item = clone_object(__DIR__"obj/flower4");
		item->move(this_object());
	}
}
*/
