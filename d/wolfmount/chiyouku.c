...
void create(){
    set("short", "蚩尤秘窟");
	set("long","迷朦的天色到这里募然骤变，刹那间浓云翻滚，电闪雷鸣。在这人畜不至的森\n"
...
int do_pull(string arg){
	...
	if(me->query("force_factor") > 80 && me->query_str() > 30 ) {
		...
		message_vision(HIY"\n菩提法杖突然放出万道金华，七色彩晕，山摇地动之间，法杖应声而起，冲天而出！\n"NOR,
				me);  
    	set("long","迷朦的天色到这里募地骤变，刹那间浓云翻滚，电闪雷鸣。在这人畜不至的森\n"
...
void stone_fall(object me){
	...
	inv = filter(all, (: interactive($1) && !$1->query_temp("marks/chiyou") :));
...
void chiyou(object ob){
	...
	chiyou = clone_object(__DIR__"npc/chiyou");
