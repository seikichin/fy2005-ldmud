...
string *book_bai = old_explode(read_file("/doc/help/book/book_bai"), "Page");

void create()
{
    set("short", "三楼");
    set("long","三楼是警世书局收藏、装订书籍的地方，不大的屋子里一排放着几张长九尺宽\n"
...
int read_book(string file)
{
		...
		input_to("selete_main",INPUT_PROMPT,"",ob,file);
		return 1;
}
	        ...
	        if (book) {
	            if (number) {
					...
					write(page(book,number));
	               	write(sprintf(HIW"─────────────────%-d─────────────────\n"NOR,
							number));
	               	input_to("back_off",INPUT_PROMPT,"",ob, file);	
	               	return;
		       	}else {
		    	    	quit_help(ob);
		        		return;	
