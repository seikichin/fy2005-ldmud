...
//	current=replace_string(quest,"，","");  // the answer
    ...
    if(sizeof(filter(all_inventory(this_object()), (:userp($1):))))
...
string mixup(string str)
{
string* poemstr,str1,str2,str3;
poemstr=explode(str,"，");
if(!sizeof(poemstr)) return str;
if(sizeof(poemstr)==1) return poemstr[0];
str1= poemstr[0];
str2=poemstr[1][0..<2];
str3=poemstr[1][<1..<1];
   for(int i=0;i<2;i++){
                int a=random(time())%sizeof(str1);
                int b=random(time())%sizeof(str1);
                string c=str1[b..b];
                str1[b..b]=str1[a..a];
                str1[a..a]=c;
        }
           for(int i=0;i<2;i++){
                int a=random(time())%sizeof(str2);
                int b=random(time())%sizeof(str2);
                string c=str2[b..b];
                str2[b..b]=str2[a..a];
                str2[a..a]=c;
        }
...
/*
	...
	length=j;
//	write("333"+length+"---"+sizeof(old_explode(read_file(filename),"\n"))+"\n");
