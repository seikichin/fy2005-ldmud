...
void create()
{
        set("short", YEL"玄武门之巅"NOR);
        set("long","琉璃玉瓦有如黄金般灿烂辉煌，远远望去，红墙之下，风起云涌，四方云动！ \n"
""
        );
	set("exits", ([ 
  		"southeast" : __DIR__"zijin1",
		"south" : __DIR__"zijin_w",
		"east" : __DIR__"zijin_e",
	]));
...
//write(sprintf("%O", fighters));
				fighters = filter(all, (: interactive($1) :));
	...
	if(arg == "southeast"){
		for(i=0; i<sizeof(fight_rooms); i++) {
			if(room = find_object(__DIR__+fight_rooms[i])) {
				...
				fighters = filter(inv, (: interactive($1) :));
				if(sizeof(fighters)!=0){
						return notify_fail("作为观众请待在这里，如果要决战请输入你要决战的人。（ｊｕｅｚｈａｎ）\n");
				}
			}
		}
		return 1;
