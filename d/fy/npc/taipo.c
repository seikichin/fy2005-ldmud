...
int chat()
{
	...
	if( in_ask || random(20) > 10 ) return 1;
	delete_temp("wrong");
...
int do_answer(string arg)
{
    ...
    // statuss_ 1: read a new number,
    //           valid input: '(' '1'-'9'
    // statuss_ 2: finished a number, (notice, all numbers should be single digit
    //           valid input: ')' + - * /
    // After read in a number, statuss_ change 1 to 2
    // After read in a oper, statuss_ change from 2 to 1,
       int statuss;
   ...
   // Scan the string now, and store it in the arrays
	s=arg;
        ...
        statuss=1; //initial statuss
        count=0; //start with no number

        while( count<5 && i<length-1)
        {
         i++;
        if (statuss==1)  //read a number, valid input: ( , 1-9
          {
           if(s[i]=='(')
             bracket[count]+=1;
           else if(s[i]>='1' && s[i]<='9')
...
//          return notify_fail(""数学老师说道：错！不等于二十四呀！\n");
       ...
       else
       {
		object ob;
          	...
          	right = 1;
    		ob = clone_object("/obj/money/coin");
    		ob->set_amount(100);
