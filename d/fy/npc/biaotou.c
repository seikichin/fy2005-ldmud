...
int random_move()
{
	...
	if( !mappingp(exits = environment()->query("exits")) ) return 0;
...
void go()
{
    	...
    	for(i=0;i<sizeof(way);i++)
    	{
        	...
        	cmds=old_explode(way[i]["cmd_table"],";");
        	break;
    	}
...
// Some players joined team after getting mission.....
    	...
    	for (i=0; i<= num; i++)
    	{
        	robber=clone_object(__DIR__"gangster");
    	...
    	if (!random(5)){
    		...
    		robber=clone_object(__DIR__"gangster_1");
        	robber->move(environment());
        	robber->set("combat_exp",exp);
        	create_gangster(this_object(),robber,"gangster",exp);
        	robber->kill_ob(this_object());
