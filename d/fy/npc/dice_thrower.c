...
//      object cash,tencash,gold,silver,coin;
        ...
        if( amount/1000000 ) {
                ob = clone_object(TECASH_OB);
                ob->set_amount(amount/1000000);
                if (!ob->move(who)) 	destruct(ob);
                amount %= 1000000;
        }
        if( amount/100000 ) {
                ob = clone_object(THCASH_OB);
                ob->set_amount(amount/100000);
                if (!ob->move(who)) 	destruct(ob);
                amount %= 100000;
        }
        if( amount/10000 ) {
                ob = clone_object(GOLD_OB);
        ...
        if( amount/100 ) {
                ob = clone_object(SILVER_OB);
                ob->set_amount(amount/100);
                if (!ob->move(who)) 	destruct(ob);
                amount %= 100;
        }
        if( amount ) {
                ob = clone_object(COIN_OB);
                ob->set_amount(amount);
                if (!ob->move(who)) 	destruct(ob);
        }
//	if (QUESTS_D->verify_quest(who,"黄麻子赌博")) 
		QUESTS_D->special_reward(who,"黄麻子赌博");        
