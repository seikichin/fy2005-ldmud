...
int do_estimate(string arg)
{
	...
	base_damage=weapon->query("base_damage");
	if(!base_damage) {
	base_damage=10;
	weapon->set("base_damage", 10);
	}
	...
	if( !weapon->query("owner") ) {
		// 通用武器修理。
    	seteuid(getuid());
		reference = clone_object(base_name(weapon));
		if( !reference ) {
		...
		// 自制武器修理：
		dam = base_damage - weapon->query("weapon_prop/damage");
		if( dam <= 0) {
			return notify_fail("你的武器没有损坏。\n");
		} else {
...
int check_legal_name(string name)
{
	int i;
	i = strlen(name);
	if( (text_width(name) < 1) || (text_width(name) > 20 ) ) {
...
int check_legal_id(string name)
{
	int i;
	i = strlen(name);
	if( (text_width(name) < 3) || (text_width(name) > 20 ) ) {
