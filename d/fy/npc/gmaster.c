...
//end: modified by justdoit
    	...
    	ob=clone_object(__DIR__"obj/cart");
    	ob->move(environment());
    	cart=ob;
	
    	biaotou=clone_object(__DIR__"biaotou");
    	...
    	ob=clone_object("/d/fy/obj/biaoyin");
    	ob->set_amount(10+random(10));
    	ob->move(cart);
	
    	ob=clone_object(__DIR__"biaoshia");
    	ob->move(environment());
    	...
    	ob=clone_object(__DIR__"biaoshi1a");
    	ob->move(environment());
    	ob->set_leader(biaotou);
    	ob->set_temp("protecting",cart);
    	ob->set("combat_exp",exp/2);
