...
//	object	con,item;
	...
	set("short", "厢房");
	set("long","武当弟子休息的地方。最里面靠墙是一排木床，大约有十几个。房子中央是一\n"
...
void reset()
{
	...
	int		i;	
        ...
        if( sizeof(inv) != 5) {
        	...
        	item = clone_object(__DIR__"obj/teapot");
        	item->move(con);
        	item = clone_object(__DIR__"obj/cup");
        	item->move(con);
        	item = clone_object(__DIR__"obj/cup");
        	item->move(con);
        }
        ...
        if( sizeof(inv) != 3) {
	        for(i=sizeof(inv)-1; i>=0; i--) destruct(inv[i]);
        	item = clone_object(__DIR__"obj/sword");
        	item->move(con);
	}
/*      con = present("book shelf", this_object());
        inv = all_inventory(con);
        if( sizeof(inv) ==0) {
        	item = clone_object(__DIR__"obj/book");
        	item->move(con);
	} */
}
