...
{        
        ...
        if( !mappingp(exit = env->query("exits")) || undefinedp(exit[dir]) ) 
        {
                if( query_verb()=="drive")
...
void to_rob(object me)
{
	...
	if( !me->query_temp("meet_poor") && environment(this_object())->query("bandit_out"))
	{
		...
		for (i=0;i<2;i++)
		{
			robber=clone_object(__DIR__"poorrobber");
			robber->move(room);
			robber->set("combat_exp",100000 + random(100000));
			robber->set_leader(me);
		}
		robber=clone_object(__DIR__"robleader");
		robber->move(room);
	...
	if (!random(8))
	{
		...
		{
			robber=clone_object(__DIR__"robber");
			robber->move(room);
