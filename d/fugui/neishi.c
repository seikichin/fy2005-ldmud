...
{
	set("short", "内室");
	set("long","这是麦老广做烧鸭的厨房，也是他休息睡觉的地方，虽然屋中到处都是厚厚的\n"
...
int entering(object me)
{
	...
	int i,j,k;
		...
		if( !sizeof(inv))
		{
	    	bot  = clone_object(__DIR__"obj/tieguo");
			ky1  = clone_object(__DIR__"obj/kaoya1");
			zb=clone_object(__DIR__"obj/xing");
			zb->move(ky1); 
			zb=clone_object(__DIR__"obj/pearl");
			zb->move(ky1); 
			zb=clone_object(__DIR__"obj/yuxi");
			zb->move(ky1); 
			zb=clone_object(__DIR__"obj/pin");
			zb->move(ky1); 
			zb=clone_object(__DIR__"obj/zhubao");
			zb->move(ky1); 
			j=random(5);
			for (i=0;i<j;i++)
			{	
				ky  = clone_object(__DIR__"obj/kaoya");
				ky->move(bot); 
			}
			ky1->move(bot);
			k=random(5);
...
int do_get(string arg)
{
	...
	if(where == "zaotai" || where == "灶台" )
	{
		...
		{
			jin = clone_object(__DIR__"npc/jinshizi");
			jia = clone_object(__DIR__"npc/jiagun");
			fig = clone_object(__DIR__"npc/meirujia");
