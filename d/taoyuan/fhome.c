...
{
        set("short", "渔家");
        set("long","简陋的宅院收拾得十分干净，土坯搭成的墙围成一个四方行的院子，院子中央\n"
...
int do_pick(string arg){
	...
	if(query("veg_amount") > 0){
		...
		if(i == 1) {
			veg = clone_object(__DIR__"obj/soybean");
		} else if(i == 2){
			veg = clone_object(__DIR__"obj/youcai");
		} else if(i == 3){
			veg = clone_object(__DIR__"obj/greenorion");
		} else if(i == 4){
			veg = clone_object(__DIR__"obj/donggua");
		}
		add("veg_amount", -1);
		veg->move(me);
	} else {
