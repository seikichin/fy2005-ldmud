...
{
	set("short", "核桃树下");
	set("long","靠近草地的边缘长着一棵枝叶繁茂的小树，树下是碧绿的草地，一块平整的大\n"
...
{	
	...
	walnut = clone_object(__DIR__"obj/walnut");
...
//	int i,age;
	...
	ppls=filter(all_inventory(ob),(:userp($1) && interactive($1) && $1->query("age")>=MINAGE && $1->query("age")<=MAXAGE && !$1->query(FATEMARK):));
	if(sizeof(ppls))
		map(ppls,f);
}
