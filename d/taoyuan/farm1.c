...
{
        set("short", "农家");
        set("long","这是一户很简朴的农家，土坯垒成的墙冬暖夏凉，茅草盖顶，虽然简陋却足以\n"
...
void reset()
{
	object *inv;
        ...
        if (!hook) {
        	item1 = clone_object(__DIR__"obj/hook");
        	item1->move(wall);
        	}
        if (!raincoat) {
        	item2 = clone_object(__DIR__"obj/raincoat");
        	item2->move(wall);
        	}
        if (!liandao) {
        	item3 = clone_object(__DIR__"obj/liandao");
        	item3->move(wall);
        	}
}
