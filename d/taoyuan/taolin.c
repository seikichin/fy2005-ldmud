...
{
        set("short", "桃林");
        set("long","这是一片茂密的桃树林，林中芳草鲜美，落英缤纷，一层淡淡的薄雾，混合着\n"
...
int do_pick(string arg)
{
	...
	i = NATURE_D->get_season();
   	...
   	if (i == 2 && (arg == "peach" || arg == "蜜桃")){
    	if(query("peach_amount") > 0) {
    		tao = clone_object(__DIR__"obj/taozi");
      		tao->move(me);
      		add("peach_amount", -1);
      		message_vision("\n$N伸手摘了一个鲜美的桃子下来。\n", me);
   		} else { 
    	...
    	if(query("flower_amount") > 0) {
    		message_vision("\n$N伸手折下一枝桃花。\n", me);
    		add("flower_amount", -1);
    		tao = clone_object(__DIR__"obj/taozhi");
    		tao->move(me);
    	} else {
    		tell_object(me, "没有花了。\n");
    	}
...
void reset()
{
   ...
   if( !sizeof(inv)) {
      item1 = clone_object(__DIR__"obj/petal");
      item2 = clone_object(__DIR__"obj/mushroom");
      item1->move(grass);
      item2->move(grass);
   }
}
