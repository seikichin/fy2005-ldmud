...
int	valid_leave(object who, string dir)
{
	...
	// get invition (set mark to 6); --- I see two ways to get letter
	// or rob the 请柬 (set mark to 7) --- ??
	if (present("shaolin monk") && dir == "southdown")
		if (!REWARD_D->check_m_success(me,"杀和尚") 
			&& !REWARD_D->check_m_success(me,"溜过去") 
			&& !REWARD_D->check_m_success(me,"请柬") )
	...
	if (dir == "southdown")
		if (REWARD_D->riddle_check(me,"韶光虚掷") >= 4) 
			if (REWARD_D->riddle_check(me,"韶光虚掷") <= 6) 
				REWARD_D->riddle_set(me,"韶光虚掷",7);
