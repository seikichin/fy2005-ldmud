...
{
	set("short", "湖畔小径");
    set("long","漫步在映景明湖畔的小径上，四周寂静无声，一边是落叶缤纷的高大树木\n"
...
int do_a1()
{
	...
	me->delete_temp("annie/answer");
		...
		target=clone_object(__DIR__"obj/letter2");
		target->move(me);
...
void init() {
	...
	if (REWARD_D->riddle_check(me,"韶光虚掷") <=6 
		&& !REWARD_D->check_m_success(me,"铭心") 
		&& !REWARD_D->check_m_success(me,"湖畔相会") 
		&& !REWARD_D->check_m_success(me,"顾曲") 
		&& !REWARD_D->check_m_success(me,"听曲") 
		&& !REWARD_D->check_m_success(me,"相会") 
		&& NATURE_D->getTime(time())[1] >3 && NATURE_D->getTime(time())[1] <6
		) 
	{
		ob=clone_object(__DIR__"npc/pet");
		
		ob->move(this_object());

		ob=clone_object(__DIR__"npc/feng");
