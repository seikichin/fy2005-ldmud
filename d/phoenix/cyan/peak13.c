...
{
	set("short", "明月故居");
    set("long","琼花劫到，玉树歌终，暮色中的小小木屋只似天地间一抹黛色，竟是极薄\n"
...
void init() {
	...
	if (REWARD_D->riddle_check(me,"韶光虚掷") <=6 
		...
		&& NATURE_D->getTime(time())[1] >7 && NATURE_D->getTime(time())[1] <11
		) 
	{

		ob=clone_object(__DIR__"npc/ruo");		
		ob->move(this_object());
...
void do_ggyy2(object me,object target,int count)
{
	string msg;
	string *event_msg = ({
"你振衣纵跃到那杨木边蹲下，双手轻环抱起紫衣少女，让她枕在臂弯之间，动作自然随性，仿佛许久\n"
"之前便有过这般经验。",
...
"苏若双指一弹将请柬射到你怀中，旋即飘然而去，身后留下一串银铃般的笑声：大少爷，可记得你又欠我一份人情噢～",
	...
	else
	{
		...
		target=clone_object(__DIR__"obj/letter1");
		target->move(me);
