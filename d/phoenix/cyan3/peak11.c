...
{
	set("short", "映景明湖");
    set("long","进山这许多天来，第一次看见湖水的欣喜刹时间满溢心头。近到湖边捧起\n"
...
object me = this_player();
	...
	{
		ob=clone_object(__DIR__"npc/dizi");
		ob->set("name","张三");
		ob->move(this_object());
		ob=clone_object(__DIR__"npc/dizi");
		ob->set("name","李四");
...
"$n拍拍$N的肩膀，纵身而起，转瞬间化作一道青烟消失在山林之间。\n众弟子向$N躬身一礼，亦四散离去。",
	...
	me->set("quest_time",time());
		...
		destruct(target);
		target=clone_object("/d/phoenix/npc/obj/cyan_drug");
		target->move(me);
...
int	valid_leave(object who, string dir)
{
	object me;
	me = who;
		if (REWARD_D->riddle_check(me,"天涯魂梦") == 3) 
		{
		...
		if (REWARD_D->riddle_check(me,"天涯魂梦") == 2) 
		{
			...
			who=clone_object(__DIR__"npc/j1");
