...
//	map_skill("parry","dragonwhip");
	...
	carry_object(__DIR__"obj/cyan_drug")->set_amount(6);
...
void get_riddle_stat(object ob, object ob1)
{
    mapping list, tlist;
    mixed *klist, *tlist2;
    string text,*tmp, id, spec;
    ...
    list = ({mapping})ob->query("annie_dream/riddle");
    tlist = ({mapping})ob->query("annie_dream/m_success");
	...
	for(loop=0; loop<sizeof(klist); loop++)
		ob1->set("riddle/"+klist[loop],list[ klist[loop] ]);

	for(loop=0; loop<sizeof(tlist2); loop++)
		ob1->set("m_success/"+tlist2[loop],tlist[ tlist2[loop] ]);
...
void die()
{
	...
	if (me->query_temp("link_ob"))
	{
		...
		{
		"/cmds/adm/_exec.c"->_exec(me->query_temp("link_ob"), me);
...
void leftnow()
{
	...
	if (me->query_temp("link_ob"))
	{
		if (me->query_temp("link_ob")->is_character())
		{
			...
			// here, we transfer our marks to "real" body
			...
			// Annie Virtual Area Support.
			// quand un joueur cesse, détruire son secteur.
				REWARD_D->riddle_set(me->query_temp("link_ob"),"红尘劫",2);
				
				me->query_temp("link_ob")->set_skill("theorem",100);
				me->query_temp("link_ob")->delete("NO_KILL");
	...
	if(result && interactive(this_object()) 
		...
		if(query("env/brief")) {
			...
			for(i=0; i<sizeof(inv); i++) {
				...
				if(!this_object()->visiblep(inv[i]) ) continue;
				str += "  " + inv[i]->short() + "\n";
			}
			tell_object(this_object(), str);
		} else {
...
void receive_message(string msg,string cl){
		::receive_message(msg,cl);
}
