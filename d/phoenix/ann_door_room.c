...
void  add_actions(string ac,mixed* m){
...
void init()
{
	...
	add_actions("do_nothing",({ "new","chat","rumor","shout",
...
query_doors gives:
	...
	DOORS will be reset to old status_ during each reset.
...
string look_door(string dir)
{
    	...
    	if( !mappingp(doors) || undefinedp(doors[dir]) )
        	return "你要看什麽？\n";
	...
	if( !undefinedp(default_dirs[arg]) )
    	...
    	if( doors[dir]["statuss"] & DOOR_CLOSED )
...
varargs int open_door(string dir, int from_other_side)
{
    	...
    	exits = query("exits");
    	if( !mappingp(exits) || undefinedp(exits[dir]) )
...
int check_door(string dir, mapping door)
{
    // If we have no responding door, assume it is correct.
    	if( !mappingp(doors) || undefinedp(doors[dir]) ) return 1;

    	door["statuss"] = doors[dir]["statuss"];
    	return 1;
}


varargs void create_door(string dir, string door_id,mixed data,string other_side_dir, int statuss)
{
...
{
    	if( !mappingp(doors) || undefinedp(doors[dir]) ) return 0;
...
int reset_single_door(string dir, int the_other_side){
	...
	doors[dir]["statuss"] = doors[dir]["old_status"];
...
int reset_door(){
	...
	if (!mappingp(doors)) return 0;
	door_exit = keys(doors);
	if (!door_exit)	return 0;
	
	for (i = 0; i< sizeof(door_exit); i++) {
		if (doors[door_exit[i]]["statuss"] != doors[door_exit[i]]["old_status"]) 
...
int valid_leave(object me, string dir)
{
        ...
        // if ( door[dir]["statuss"] & DOOR_HAS_TRAP ) ....
    	}
    	return 1;
}
...
int	do_look(string arg)
{
	...
	exits = query("exits");
	if (!mappingp(exits) || sizeof(exits)<1)	return 0;
