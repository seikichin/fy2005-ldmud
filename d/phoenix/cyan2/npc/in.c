...
//	who->init_me(me,"garrison",2);
	...
	if (dif == 2)
		exp=exp*9/10;
	if (dif >= 3)
//		exp=exp*(10+dif/3)/10;		Modification 1: Same exp as player
		exp = exp;
...
/*	set("resistance/kee",exp/333333);
	...
	set("max_kee",query("max_kee")*3/2);
	set("max_gin",query("max_gin")*3/2);
	set("max_sen",query("max_sen")*3/2);
...
//	ccommand("say kee = "+HIR+query("kee")+NOR+" ,str = "HIC+query_str()+NOR" ,resist = \n");
