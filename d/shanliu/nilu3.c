...
void create()
{
	...
	set("long","这是条泥土路，似乎刚下过雨，山路也格外泥泞难行，你低一脚高一脚往前努\n"
...
{
	object me;
	if (interactive(me=this_player()) && userp(me) && (me->query_temp("入山流/送东西") || (mappingp(me->query("organization")) && ({string})me->query("organization/org_name")=="山流")))
	{
...
void killshanliu(object me)
{
	...
	if (!ayan)
	{
		ayan = clone_object(__DIR__"npc/tianzong3");
