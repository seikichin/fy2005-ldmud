...
void create()
{
	...
	set("long","这是个看上去很老的和尚，长得胖胖的，笑起来简直就活脱脱一个弥勒佛，面前还\n"
"摆了个木鱼。\n");
        set("chat_chance", 1);
        set("chat_msg", ({
...
void greeting(object ob)
{
	...
	if(mappingp(ob->query("organization")) && ({string})ob->query("organization/org_name")=="山流" )
		return;
...
"去梵音寺的山道中的牛肉铺子老板。切勿私自拆看，切记。”\n"NOR,this_object(),me);
		letter=clone_object(__DIR__"obj/letter");
		letter->move(me);
		letter->set("送信人",me);
		me->set_temp("山流/送信",1);
		delete("answering_person");
...
int answer_shanliu()
{
	...
	me=this_player();	
	if(mappingp(me->query("organization")) && ({string})me->query("organization/org_name")=="山流" )
	{
