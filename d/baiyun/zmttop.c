...
{
        set("short", "斩蟒台顶");
        set("long","此处便是斩蟒台顶。面北而立，碧海蓝天，云雾氤氲，一叶扁舟在浪涛上几个颠\n"
...
void shoot_me(object me)
{
	object box,cloth,seal,hat;
        ...
        message_vision(HIW"\n\n$N神色一凛，小心地把描金箱子收入怀中。\n"NOR,me);
        box=clone_object(__DIR__"obj/goldbox");
        cloth=clone_object(__DIR__"obj/emperor_cloth");
        seal=clone_object(__DIR__"obj/jade_seal");
        hat=clone_object(__DIR__"obj/sinny_hat");
        cloth->move(box);
        seal->move(box);
        hat->move(box);
        box->move(me);
