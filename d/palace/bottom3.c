...
{
        set("short", "湖底");
        set("long","这里是一个静悄悄的世界，你几乎忘记了外面世界的喧哗。色彩鲜艳，形状个\n"
...
void reset()
{
	...
	watergrass = present("watergrass", this_object());
        ...
        if(!sizeof(inv)) {
	con = clone_object(__DIR__"obj/woodcase");
	item = clone_object(__DIR__"obj/ring");
	item->move(con);
	if (!random(4)) {
		item = clone_object("obj/medicine/pill_medicine");
		item->move(con);
		}
	con->move(watergrass);
	}
