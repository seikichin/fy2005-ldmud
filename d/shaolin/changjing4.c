...
void create()
{
	object ob1,ob2,con;
        set("short", "藏经阁");
        set("long","藏经楼顶，这里的每件物品都是稀世之宝，底下三层还只是略加护卫，而这里\n"
...
""
	...
	setup();
	con = clone_object(__DIR__"obj/he");
	ob1 = clone_object(BOOKS"chant_75");
	ob2 = clone_object(BOOKS"skill/yoga_0");
	if(con && ob1 && ob2)
	{
		ob1->move(con); 
		ob2->move(con); 
...
int stealing(object me)
{
	...
	if(environment(me) == this_object() && !present("mysterious figure",this_object()))
	{
		...
		{
			ob = clone_object(__DIR__"npc/master_17_7");
