...
void create()
{
        ...
        set("long","少林寺七大高手之一，也是达摩院首座，为寺中众僧所敬重。\n");
    	create_family("少林寺", 17, "达摩院首座");
    	set("class", "shaolin");
    	...
    	set("perform_weapon_attack","shaolin/spear/duanhun-spear/suomengchanhun");
    	set("smartnpc_busy",1);	
...
int choice()
{
	...
	if (me->query("marks/damoyuan2") == "xinzhu"){
...
"老衲研习断魂枪法已有四十二年，穷心极力，虽略有小成，只是在佛法\n"
"修为上，却及不上方丈师兄明心见性之无我之境，所谓禅道为本，武道\n"
"为辅。师侄既是方丈师兄所重，想必宿根深厚，大智大慧，老衲不敢为\n"
"师。若师侄有心，老衲可与你相互切磋。\n"NOR, this_object());
	tell_object(this_player(),"（达摩四僧，各有所长，你是否选择先向心烛学艺？answer yes/no）\n");
	tell_object(this_player(),"（你可用skills来先看一下达摩四僧的武功）\n");
	this_player()->set_temp("answer/damoyuan2",1);
	return 1;
}
...
int do_answer(string arg)
{
	...
	if ( !this_player()->query_temp("answer/damoyuan2"))
		return 0;
	
	if (arg == "no")
	{
		this_player()->delete_temp("answer/damoyuan2",1);
		message_vision(CYN"$N微微一笑，不再言语。\n"NOR, this_object());
	}
	else if (arg == "yes")
	{
		message_vision(CYN"$N说道：有心皆苦，无心即乐，老衲愿与师侄共勉。\n"NOR,this_object());
		this_player()->set("marks/damoyuan2","xinzhu");
		this_player()->delete_temp("answer/damoyuan2",1);
	}else
...
{
	if(ob->query("marks/damoyuan2") == "xinzhu")
		return 1;        
}
