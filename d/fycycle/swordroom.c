...
{
	set("short", "江海清光");
	set("long","眼前晶光耀目，如同江海浮动的清波。你定睛细看，发现全屋竟然都是明玉\n"
...
void reset()
{
	...
	if( !sizeof(inv)) 
	{
		item = clone_object(__DIR__"obj/forcebook");
		item->move(endtable);
		item = clone_object(__DIR__"obj/movebook");
		item->move(endtable);
		item = clone_object(__DIR__"obj/swordbook");
		item->move(endtable);
		item1 = clone_object(__DIR__"obj/" + book);
		item1->move(endtable);
	}
	::reset();
}
