...
{
        set("short", "银杏溪边");
        set("long","溪流不宽，紧挨小溪生长着一棵巨大的银杏树，树粗有数合抱，树冠如盖十分\n"
...
void snake_come(object me){
	...
	message_vision(MAG"\n一条全身紫红色的小蛇从古藤中游走了出来。\n"NOR, me);
	obj = clone_object(__DIR__"npc/snake");
...
int do_pick(string arg){
	...
	if((i == 1 || i == 2) && query("leaf_amount") > 0 && (arg == "leaf" || arg == "叶")){
		...
		obj = clone_object(__DIR__"obj/leaf"); 
		if(objectp(obj)){
			obj->move(me);
		}
		add("leaf_amount", -1);
		...
		obj = clone_object(__DIR__"obj/ginkgo");
		if(objectp(obj)){
			obj->move(me);
		}
		add("baiguo_amount", -1);	
	...
	if(query("have_herb") > 0 && (arg == "grass" || arg == "小草")) {
		...
		obj = clone_object(__DIR__"obj/herb");
		if(objectp(obj)){
			obj->move(me);
		}
		add("have_herb", -1);
