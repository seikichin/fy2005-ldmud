...
{
        set("short", "山崖底");
        set("long","山崖，在这里洒下了它灰暗的影子。异常的安静让你不由自主心神高度紧张，\n"
...
string deadwolf()
{
	...
	if( me->query_temp("followcrow")) 
	{
		...
		{
			obj = clone_object(__DIR__"obj/deadwolf");
...
int do_search(){
	...
	if(random(j) > 20 && query("jade_bottle")){
		...
		obj = clone_object(__DIR__"obj/bottle");
		if(objectp(obj)){
			obj->move(me);
		}
		add("jade_bottle", -1);
...
int do_pick(string arg){
	...
	flower = clone_object(__DIR__"obj/redflower");
	if(objectp(flower)){
		flower->move(me);
	}
	add("flower_amount", -1);
