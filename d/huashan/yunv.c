...
{
        set("short", "玉女峰");
        set("long","一路行来风景秀丽，峰上更是林木茂密，环境清幽。巨石边，小溪侧奇花异草\n"
...
int music_npc(object me){
	...
	if (REWARD_D->riddle_check(me,"听琴毛女洞") == 3)
	{
		...
		obj = clone_object(__DIR__"npc/qing");
		obj->delete("chat_msg");
		poison();
		...
		|| REWARD_D->check_m_success(me,"听琴毛女洞"))
	{
		...
		obj = clone_object(__DIR__"npc/zhenzhen");
	}
	if(objectp(obj))
...
int poison(){
	...
	int i;
	inv = all_inventory();
    players = filter(inv, (: interactive($1) :));
...
"凡人是不会有这么轻妙的身法的。\n", me);
		obj = clone_object(__DIR__"npc/eagleman");
		obj->move(this_object());
		obj->wangyou(me);
	}
	return 1;
