...
void create()
{
	...
	set("long","青青穿着一身淡青色的衣服，就像是春天晴朗的天空，晴空下清澈的湖 \n"
...
void xiaolou(object me){
	...
	message_vision("\n$N的脸色变了，忽然抛下了手里的花蓝，握住了那柄弯刀的刀柄。\n",
			this_object()); 
	petal = clone_object(__DIR__"obj/petal");
...
+ "放在$n怀里。\n", this_object(), me);
		...
		if (blade && REWARD_D->riddle_check(me,"听琴毛女洞") == 3)
		{
			...
			blade=clone_object(__DIR__"obj/grass");
			blade->move(me);
			blade->set("owner",me);
		}
...
void die(){
	...
	obj = clone_object(__DIR__"obj/basket");
