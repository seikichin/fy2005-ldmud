...
{
        set("short", "驿站马房");
        set("long","人须健儿，马须良驹，每个驿站都配有专门的马房。这儿地处西疆，马都是\n"
...
void reset(){
	...
	if (!present("broken rake",coffin)) {
		item=clone_object(__DIR__"obj/brokenrake");
		item->move(coffin);
	}
	if (!present("gao",coffin)) {
		item=clone_object(__DIR__"obj/gao");
		item->move(coffin);
	}
	set("mouse",5);
	return;
...
"辛苦了一番，马房里终于干净了一些。\n\n"NOR, me);
        ...
        if (!random(5) && query("mouse")){
        	mouse=clone_object(__DIR__"npc/mice");
