...
{
        set("short", "书房");
        set("long","这里就是白三空的书房，里面足足两柜的书，白三空武功修为已臻化境，没想\n"
...
//	   object *inv;
	   ...
	   if (!present("silk bag", desk)) {
	   		item2 = clone_object(__DIR__"obj/bag");
	      	item2->move(desk);
	   }
	   	
	   if (!present("letter", desk))	{
			item1 = clone_object(__DIR__"obj/letter");
	      	item1->move(desk);
	   }
	   
	   if (!present("kite",wall)) {
	   		item1 = clone_object(__DIR__"npc/obj/kite");
	      	item1->move(wall);
	   }
