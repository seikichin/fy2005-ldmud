...
void init_scenery()
{
        ...
        call_out((:sun_rise(1, 0):),random(2));
}
    	...
    	if(sunrise == 0)
    	{
        	switch(sunrise_stage)
        	{
            		...
            		case 2:
		    			...
		    			ppls=filter(all_inventory(ob),(:userp($1) && interactive($1) && !$1->query(FATEMARK):));
                    		    	if(sizeof(ppls))
						map(ppls,f);
	                	}
                break;
        	default:
        		 notify_fail("something wrong\n");
        		 return ;
        }
    } else
    {
