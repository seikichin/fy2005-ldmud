...
{
	set("short", "水梦庵");
    set("long","这座小庵隐在乱石之后，不为世人所知。这里供着的是一座观世音菩萨像\n"
...
void reset()
{
	...
	if (sizeof(inv) < 1)
	{
		stone = clone_object(__DIR__"obj/willow");
		if(stone)
			stone->move(river);
	}
	...
	if (sizeof(inv) < 1)
	{
		stone = clone_object(__DIR__"obj/nbook");
		if(stone)
			stone->move(river);
		stone = clone_object(__DIR__"obj/nbook1");
		if(stone)
			stone->move(river);
	}
...
""
	...
	if (sizeof(inv) < 1)
	{
		stone = clone_object(__DIR__"obj/nbook2");
		if(stone)
			stone->move(river);
		stone = clone_object(__DIR__"obj/nbook3");
		if(stone)
			stone->move(river);
	}
