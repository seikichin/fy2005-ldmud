...
{
	set("short", "星月斋");
    set("long","走进此处，你心头赫然一惊。好特殊的楼宇，宽敞如深邃的大海，不知是\n"
...
void reset()
{
	...
	if (sizeof(inv) < 1 )
	{
		stone = clone_object(__DIR__"obj/wine5");
		if(stone)
			stone->move(river);
		stone = clone_object(__DIR__"obj/wine1");
		if(stone)
			stone->move(river);
		stone = clone_object(__DIR__"obj/wine2");
		if(stone)
			stone->move(river);
		stone = clone_object(__DIR__"obj/wine3");
		if(stone)
			stone->move(river);
		stone = clone_object(__DIR__"obj/wine4");
		if(stone)
			stone->move(river);
	}
