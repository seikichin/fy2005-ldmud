...
{
        set("short", "温泉");
        set("long","黄山温泉古称汤泉，古人云“紫云峰下藏太阳，不施炉灶自成汤”，更兼有医\n"
...
int do_pick(string arg)
{
	...
	if( i==1 || i == 2){  
   		...
   		{
   			if (random(luck) > 15) {
				banana = clone_object(__DIR__"obj/gbanana");
      			banana->move(me);
      		} else {
      			banana = clone_object(__DIR__"obj/banana");
      			banana->move(me);
      		}
      		add("banana_amount", -1);
...
void durian_fall(object me)
{
	...
	add("durian_amount", -1);
	if(random(luck) > 10) {
		durian = clone_object(__DIR__"obj/gdurian");
		durian->move(this_object());	
	} else {
		durian = clone_object(__DIR__"obj/durian");
		durian->move(this_object());
	}
}
