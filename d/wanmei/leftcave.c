...
{
        set("short", "左边小洞");
        set("long","钻进洞中抬头望去，黑暗中依稀看到山洞顶部凹凸不平，上面有许多草藤交\n"
...
void reset(){
	...
	if(!present("icy boots", this_object())){
		item = clone_object(__DIR__"obj/icy_boot");
	        if(objectp(item)){
                	item->move(this_object());
                }
        }
        if(!present("icy ribbon", this_object())){
		item = clone_object(__DIR__"obj/icy_ribbon");
	        if(objectp(item)){
                	item->move(this_object());
                }
        }
        if(!present("icy girth", this_object())){
		item = clone_object(__DIR__"obj/icy_girth");
	        if(objectp(item)){
                	item->move(this_object());
                }
        }
        if(!present("icy cloth", this_object())){
		item = clone_object(__DIR__"obj/icy_cloth");
	        if(objectp(item)){
                	item->move(this_object());
                }
        }
