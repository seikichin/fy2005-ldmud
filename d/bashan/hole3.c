...
{
	set("short", "断崖平台");
        set("long","自山崖上攀爬下到这里，喘息良久方定。抬眼回望，只见不知觉中竟爬下了数\n"
...
//		return;
	...
	if (count == 0)
	{
		me = clone_object(__DIR__"npc/shui");
		me->move(env);
...
void get_taoist()
{
	...
	for (i=0; i<8;i++ )
	{
		ob=clone_object(__DIR__"npc/taoist");
		ob->move(this_object());
	}
...
void get_monk()
{
	...
	for (i=0; i<4;i++ )
	{
		ob=clone_object(__DIR__"npc/monk");
		ob->move(this_object());
	}
...
void get_nun()
{
	...
	for (i=0; i<3;i++ )
	{
		ob=clone_object(__DIR__"npc/nun");
		ob->move(this_object());
	}
...
{
object ob;
	ob=clone_object(__DIR__"npc/cici");
	ob->move(this_object());
	ob=clone_object(__DIR__"npc/khl");
	ob->move(this_object());
...
void get_tx()
{
	...
	int i ;
	ob=clone_object(__DIR__"npc/txmale");
	ob->move(this_object());
	ob=clone_object(__DIR__"npc/txfemale");
	ob->move(this_object());
