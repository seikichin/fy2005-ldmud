...
*/
	...
	data=ANNIE_D->lotto_checkout1(me->query("id"));
	r1=data["r1"];
	r2=data["r2"];
	r3=data["r3"];
	r4=data["r4"];
...
/*	arg += "一等奖"+r2+"注，得到";
	...
	for (i=0;i<r2 ;i++ )
	{
		obj = clone_object(l2_reward[random(sizeof(l2_reward))]);
		obj->move(me);
...
int listx()
{
	...
	data = ANNIE_D->lotto_getlist1(me->query("id"));
...
int lotto()
{
	...
	input_to((:selete_main($1,me,file,1):),4,"");
...
void selete_main (string num, object me, string file,int count)
{
	...
	{
		input_to((:selete_main($1,me,file,count):),4,"");
		return ;
	}
